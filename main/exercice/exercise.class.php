...
class Exercise
{
    ...
    public function save($type_e = '')
    {
        ...
        if ($id) {
            ...
            if ($this->specialCategoryOrders) {
                $sql .=", question_selection_type= ".intval($this->getQuestionSelectionType());
            }
                    ...
                    if ($from_database) {
                        ...
                        while ($a_answers = Database::fetch_array($res_answers)) {
                            ...
                            if (!empty($s_user_answer)) {
                                $selectedAnswer = isset($real_list[$i_answer_id]) ? $real_list[$i_answer_id] : '';
                                if ($s_user_answer == $i_answer_correct_answer) {
                                    $questionScore  += $i_answerWeighting;
                                    $totalScore     += $i_answerWeighting;
                                    $user_answer = '<span>'.$selectedAnswer.'</span>';
                                } else {
                                    $user_answer = '<span style="color: #FF0000; text-decoration: line-through;">'.$selectedAnswer.'</span>';
                                }
                            }
                        ...
                        $numAnswer = $objAnswerTmp->selectAutoId($answerId);
                        if ($answerCorrect) {
                            $studentAnswerValue = isset($answer_matching[$choice[$numAnswer]]) ? $answer_matching[$choice[$numAnswer]] : '';
                            if ($answerCorrect == $choice[$numAnswer]) {
                                $questionScore  += $answerWeighting;
                                $totalScore     += $answerWeighting;
                                $user_answer = '<span>'.$studentAnswerValue.'</span>';
                            } else {
                                $user_answer = '<span style="color: #FF0000; text-decoration: line-through;">'.$studentAnswerValue.'</span>';
                            }
