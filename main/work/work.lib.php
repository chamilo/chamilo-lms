...
function display_action_links($id, $cur_dir_path, $show_tool_options, $display_upload_link, $action)
{
    ...
    if (!empty($id)) {
        ...
        ).'&origin='.$origin.'&gradebook='.$gradebook.'&id='.$my_back_id.'">'.Display::return_icon(
            'back.png',
    ...
    if ($show_tool_options && api_is_allowed_to_edit(null, true) && $origin != 'learnpath') {
        ...
        if (empty($id)) {
            $display_output .= '<a href="'.api_get_self().'?'.api_get_cidreq(
            ).'&amp;action=create_dir&origin='.$origin.'&gradebook='.$gradebook.'">';
        ...
        if (empty($id)) {
            // Options
            $display_output .= '<a href="'.api_get_self().'?'.api_get_cidreq(
            ).'&amp;action=settings&amp;origin='.$origin.'&amp;gradebook='.$gradebook.'">';
            $display_output .= Display::return_icon(
                'settings.png',
    ...
    if ($display_upload_link && api_is_allowed_to_session_edit(false, true) && !empty($id)) {
        $display_output .= '<a href="'.api_get_self().'?'.api_get_cidreq(
        ).'&id='.$id.'&action=upload_form&origin='.$origin.'&gradebook='.$gradebook.'">';
        $display_output .= Display::return_icon(
            'upload_file.png',
    ...
    if (api_is_allowed_to_edit(null, true)) {
        ...
        if (!empty($id)) {
            if (empty($_GET['list']) or Security::remove_XSS($_GET['list']) == 'with') {
                $display_output .= '<a href="'.api_get_self().'?'.api_get_cidreq(
                ).'&amp;id='.$id.'&amp;curdirpath='.$cur_dir_path.'&amp;origin='.$origin.'&amp;gradebook='.$gradebook.'&amp;list=without">'.
                    Display::return_icon(
                        'exercice_uncheck.png',
                        get_lang('ViewUsersWithoutTask'),
                        '',
                        ICON_SIZE_MEDIUM
                    )."</a>\n";
                $count = get_count_work($id);
                if ($count > 0) {
                    $display_output .= '<a href="downloadfolder.inc.php?id='.$id.'">'.Display::return_icon(
                        'save_pack.png',
                        get_lang('Save'),
                ...
                $display_output .= '<a href="'.api_get_self().'?'.api_get_cidreq(
                ).'&amp;id='.$id.'&amp;curdirpath='.$cur_dir_path.'&amp;origin='.$origin.'&amp;gradebook='.$gradebook.'&amp;list=with">'.
                    Display::return_icon(
                        'exercice_check.png',
                        get_lang('ViewUsersWithTask'),
                        '',
                        ICON_SIZE_MEDIUM
                    )."</a>\n";
                $display_output .= '<a href="downloadfolder.inc.php?id='.$id.'">'.Display::return_icon(
                ...
                if (!isset($_GET['action']) || (isset($_GET['action']) && $_GET['action'] != 'send_mail')) {
                    $display_output .= '<a href="'.api_get_self().'?'.api_get_cidreq(
                    ).'&amp;id='.$id.'&amp;curdirpath='.$cur_dir_path.'&amp;origin='.$origin.'&amp;gradebook='.$gradebook.'&amp;list=without&amp;action=send_mail&amp;sec_token='.$token.'">'.
                        Display::return_icon('mail_send.png', get_lang('ReminderMessage'), '', ICON_SIZE_MEDIUM)."</a>";
                } else {
                    $display_output .= Display::return_icon(
                        'mail_send_na.png',
                        get_lang('ReminderMessage'),
                        '',
                        ICON_SIZE_MEDIUM
                    );
                }
...
function display_tool_options($uploadvisibledisabled, $origin)
{
    ...
    echo '<form class="form-horizontal" method="post" action="'.api_get_self(
    ).'?origin='.$origin.'&gradebook='.$gradebook.'&action=settings">';
    ...
    echo '<div class="row">
				<div class="formw">
					<button type="submit" class="save" name="changeProperties" value="'.get_lang('Ok').'">'.get_lang(
        'Ok'
    ).'</button>
				</div>
			</div>';
...
<div class="control-group">
    ...
    <div class="controls">
        <label class="radio" for="uploadvisibledisabled_1">
            <input id="uploadvisibledisabled_1" class="checkbox" type="radio" name="uploadvisibledisabled"
                   value="0"   <?php if ($uploadvisibledisabled == 0) {
                echo 'checked';
            } ?> />
            <?php echo get_lang('NewVisible'); ?>
        </label>
        <label class="radio" for="uploadvisibledisabled_2">
            <input id="uploadvisibledisabled_2" class="checkbox" type="radio" name="uploadvisibledisabled"
                   value="1" <?php if ($uploadvisibledisabled == 1) {
                echo 'checked';
            } ?> />
...
<div class="control-group">
    ...
    <div class="controls">
        <label class="radio" for="student_delete_own_publication_2">
            <input id="student_delete_own_publication_2" class="checkbox" type="radio"
                   name="student_delete_own_publication"
                   value="1" <?php if ($current_course_setting_value == 1) {
                echo 'checked';
            } ?> />
            <?php echo get_lang('Yes'); ?>
        </label>
        <label class="radio" for="student_delete_own_publication_1">
            <input id="student_delete_own_publication_1" class="checkbox" type="radio"
                   name="student_delete_own_publication" value="0"
                <?php if ($current_course_setting_value == 0) {
                echo 'checked';
            } ?> />
...
{
    return $_POST[$group]['year'].'-'.two_digits($_POST[$group]['month']).'-'.two_digits(
        $_POST[$group]['day']
    ).' '.two_digits($_POST[$group]['hour']).':'.two_digits($_POST[$group]['minute']).':00';
}
...
function create_group_date_select($prefix = '')
{
    ...
    $group_name[] = FormValidator :: createElement(
        'select',
        $prefix.'day',
        '',
    ...
    $group_name[] = FormValidator :: createElement(
        ...
        $prefix.'month',
        '',
        array_combine(range(1, 12), api_get_months_long())
    );
    $group_name[] = FormValidator :: createElement(
        'select',
        $prefix.'year',
        '',
        array($d_year => $d_year, $d_year + 1 => $d_year + 1)
    );
    $group_name[] = FormValidator :: createElement(
        'select',
        $prefix.'hour',
        ...
        foreach ($work_parents as $work_parent) {
            ...
            if ($is_allowed_to_edit && $locked == false) {
                ...
                if (!empty($edit_dir) && $edit_dir == $id2) {
                    ...
                    $form_folder = new FormValidator('edit_dir', 'post', api_get_self(
                    ).'?origin='.$origin.'&gradebook='.$gradebook.'&edit_dir='.$id2);
                    ...
                    $defaults = array(
                        'dir_name' => Security::remove_XSS($my_title),
                        'description' => Security::remove_XSS($row['description'])
                    );
                    $form_folder->add_html_editor(
                        'description',
                        get_lang('Description'),
                        false,
                        false,
                        array('ToolbarSet' => 'work', 'Width' => '80%', 'Height' => '200')
                    );
                    ...
                    if ($homework['ends_on'] != '0000-00-00 00:00:00') {
                        ...
                        $form_folder->addElement(
                            ...
                            'enableEndDate',
                            null,
                            get_lang('EnableEndDate'),
                            'onclick="javascript: if(this.checked){document.getElementById(\'option2\').style.display = \'block\';}else{document.getElementById(\'option2\').style.display = \'none\';}"'
                        );
                    ...
                    $form_folder->addElement(
                        ...
                        null,
                        get_lang('AllowTextAssignments')
                    );
                    $form_folder->addElement('html', '</div>');
                    $form_folder->addElement(
                        'style_submit_button',
                        'submit',
                        get_lang('ModifyDirectory'),
                        'class="save"'
                    );
                    ...
                    if (!empty($row['qualification'])) {
                        $defaults = array_merge(
                            $defaults,
                            array('qualification[qualification]' => $row['qualification'])
                        );
                    }
                    ...
                    if ($form_folder->validate()) {
                        ...
                        if ($edit_check) {
                            ...
                            $expires_query = ' SET expires_on = '."'".($there_is_a_expire_date ? api_get_utc_datetime(
                                get_date_from_group('expires')
                            ) : '0000-00-00 00:00:00')."'";
                            Database::query(
                                'UPDATE '.$work_assigment.$expires_query.' WHERE c_id = '.$course_id.' AND id = '."'".$row['has_properties']."'"
                            );
                            ...
                            $ends_query = ' SET ends_on = '."'".($there_is_a_end_date ? api_get_utc_datetime(
                                get_date_from_group('ends')
                            ) : '0000-00-00 00:00:00')."'";
                            Database::query(
                                'UPDATE '.$work_assigment.$ends_query.' WHERE c_id = '.$course_id.' AND id = '."'".$row['has_properties']."'"
                            );
                            $sql_add_publication = "UPDATE ".$work_table." SET has_properties  = '".$row['has_properties']."', view_properties=1 WHERE c_id = $course_id AND id ='".$row['id']."'";
                            Database::query($sql_add_publication);

                            $qualification_value = isset($_POST['qualification']['qualification']) && !empty($_POST['qualification']['qualification']) ? intval(
                                $_POST['qualification']['qualification']
                            ) : 0;
                            ...
                            $sql = 'UPDATE '.$work_table.' SET
                                                 allow_text_assignment = '."'".intval(
                                $_POST['allow_text_assignment']
                            )."'".' ,
                                                 title = '."'".Database::escape_string($_POST['dir_name'])."'".',
                                                 description = '."'".Database::escape_string($_POST['description'])."'".',
                                                 qualification = '."'".Database::escape_string(
                                $_POST['qualification']['qualification']
                            )."'".',
                            ...
                            if (isset($_POST['make_calification']) && $_POST['make_calification'] == 1 && !empty($_POST['category_id'])) {
                                if (empty($link_id)) {
                                    add_resource_to_course_gradebook(
                                        ...
                                        $row['id'],
                                        $_POST['dir_name'],
                                        (float)$_POST['weight']['weight'],
                                        (float)$_POST['qualification']['qualification'],
                                        $_POST['description'],
                                        1,
                                        api_get_session_id(),
                                        $link_id
                            ...
                            if ($_POST['enableExpiryDate'] == '1') {
                                $end_date = Database::escape_string(
                                    api_get_utc_datetime(get_date_from_group('expires'))
                                );
                            }
            ...
            if (!empty($display_edit_form) && !empty($edit_dir) && $edit_dir == $id2) {
                ...
                ).'&origin='.$origin.'&gradebook='.$gradebook.'">'.$icon.'</a>';
                ...
                //if (api_is_allowed_to_edit()) {
                if ($cant_files > 0) {
                    $zip = '<a href="downloadfolder.inc.php?id='.$work_data['id'].'">'.Display::return_icon(
                        'save_pack.png',
                        get_lang('Save'),
                        array('style' => 'float:right;'),
                        ICON_SIZE_SMALL
                ...
                //}
                $url = $zip.'<a href="'.api_get_self().'?'.api_get_cidreq(
                ).'&origin='.$origin.'&gradebook='.Security::remove_XSS(
                    $_GET['gradebook']
                ).'&id='.$work_data['id'].'"'.$class.'>'.
            ...
            if ($direc_date != '' && $direc_date != '0000-00-00 00:00:00') {
                ...
                $row[] = date_to_str_ago($direc_date_local).'<br /><span class="dropbox_date">'.api_format_date(
                    $direc_date_local
                ).'</span>';
            } else {
            ...
            if ($origin != 'learnpath') {
                if ($is_allowed_to_edit) {
                    if (api_resource_is_locked_by_gradebook($id2, LINK_STUDENTPUBLICATION)) {
                        ...
                        ).'&origin='.$origin.'&gradebook='.$gradebook.'&edit_dir='.$id2.'">'.Display::return_icon(
                            'edit.png',
                            get_lang('Modify'),
                            array(),
                            ICON_SIZE_SMALL
                        ...
                        ).'&origin='.$origin.'&gradebook='.$gradebook.'&delete_dir='.$id2.'" onclick="javascript:if(!confirm('."'".addslashes(
                            api_htmlentities(get_lang('ConfirmYourChoice'), ENT_QUOTES)
                        )."'".')) return false;" title="'.get_lang('DirDelete').'"  >'.Display::return_icon(
                            'delete.png',
                            get_lang('DirDelete'),
                            '',
                            ICON_SIZE_SMALL
...
function build_work_move_to_selector($folders, $curdirpath, $move_file, $group_dir = '')
{
    ...
    $form = '<form class="form-horizontal" name="move_to_form" action="'.api_get_self(
    ).'?gradebook='.$gradebook.'&curdirpath='.Security::remove_XSS($curdirpath).'" method="POST">';
    ...
    $js = '<script>
			function plus() {
				if(document.getElementById(\'options\').style.display == \'none\') {
					...
					document.getElementById(\'plus\').innerHTML=\'&nbsp;'.Display::return_icon(
        'div_hide.gif',
        get_lang('Hide', ''),
        array('style' => 'vertical-align:middle')
    ).'&nbsp;'.addslashes(get_lang('AdvancedParameters', '')).'\';
				} else {
        ...
        array('style' => 'vertical-align:middle')
			...
			function validate() {
				...
				if(new_dir=="") {
					...
					document.form1.action = "work.php?'.api_get_cidreq(
    ).'&origin='.$origin.'&gradebook='.(empty($_GET['gradebook']) ? '' : 'view').'";
...
function get_count_work($work_id)
{
    ...
    if (!empty($group_id)) {
        $extra_conditions = " work.post_group_id = '".intval(
            $group_id
    ...
    $sql = "SELECT  count(*) as count ".
        ...
        " WHERE $extra_conditions $condition_session ";
    $result = Database::query($sql);
...
function get_work_user_list($start, $limit, $column, $direction, $work_id, $where_condition)
{
    ...
    if (!empty($work_data)) {
        ...
        while ($work = Database::fetch_array($result, 'ASSOC')) {
            ...
            if (!empty($work_assignment['expires_on']) && $work_assignment['expires_on'] != '0000-00-00 00:00:00' && $time_expires && ($time_expires < api_strtotime(
                $work['sent_date'],
                'UTC'
            ))
            ) {
            ...
            if (($can_read && $work['accepted'] == '1') || ($is_author && $work['accepted'] == '1') || $is_allowed_to_edit) {
                ...
                //Type
                $work['type'] = build_document_icon_tag('file', $work['title']);

                //File name
                $link_to_download = null;

                if ($work['contains_file']) {
                    $link_to_download = '<a href="download.php?id='.$item_id.'">'.Display::return_icon(
                        'save.png',
                        get_lang('Save'),
                        array(),
                        ICON_SIZE_SMALL
                    ...
                    $link_to_download = '<a href="view.php?id='.$item_id.'">'.Display::return_icon(
                        'default.png',
                        get_lang('View'),
                        array(),
                        ICON_SIZE_SMALL
                ...
                $work['sent_date'] = date_to_str_ago(
                    api_get_local_time($work['sent_date'])
                ).' '.$add_string.'<br />'.$work_date;
                ...
                if ($is_allowed_to_edit) {
                    if ($locked) {
                        ...
                        if ($qualification_exists) {
                            $action .= '<a href="'.$url.'&amp;action=edit&item_id='.$item_id.'&amp;parent_id='.$work['parent_id'].'" title="'.get_lang(
                                'Modify'
                            ).'"  >'.
                                Display::return_icon(
                                    'rate_work.png',
                            ...
                            $action .= '<a href="'.$url.'&amp;action=edit&item_id='.$item_id.'&gradebook='.Security::remove_XSS(
                                $_GET['gradebook']
                            ).'&amp;parent_id='.$work['parent_id'].'" title="'.get_lang('Modify').'"  >'.
                    ...
                    if ($work['contains_file']) {
                        if ($locked) {
                            ...
                            $action .= '<a href="'.$url.'&amp;action=move&item_id='.$item_id.'" title="'.get_lang(
                                'Move'
                            ).'">'.Display::return_icon('move.png', get_lang('Move'), array(), ICON_SIZE_SMALL).'</a>';
                        }
                    }
                    if ($work['accepted'] == '1') {
                        $action .= '<a href="'.$url.'&amp;action=make_invisible&item_id='.$item_id.'&amp;'.$sort_params.'" title="'.get_lang(
                            'Invisible'
                        ).'" >'.Display::return_icon(
                            'visible.png',
                            get_lang('Invisible'),
                        ...
                        $action .= '<a href="'.$url.'&amp;action=make_visible&item_id='.$item_id.'&amp;'.$sort_params.'" title="'.get_lang(
                            'Visible'
                        ).'" >'.Display::return_icon(
                            'invisible.png',
                            get_lang('Visible'),
                    ...
                    if ($locked) {
                        ...
                        $action .= '<a href="'.$url.'&amp;action=delete&amp;item_id='.$item_id.'" onclick="javascript:if(!confirm('."'".addslashes(
                            api_htmlentities(get_lang('ConfirmYourChoice'), ENT_QUOTES)
                        )."'".')) return false;" title="'.get_lang('Delete').'" >'.Display::return_icon(
                            'delete.png',
                            get_lang('Delete'),
                            '',
                            ICON_SIZE_SMALL
                    ...
                    if (api_is_allowed_to_session_edit(false, true)) {
                        $action .= '<a href="'.$url.'&amp;action=edit&item_id='.$item_id.'" title="'.get_lang(
                            'Modify'
...
{
    return $_POST[$prefix.'_year'].'-'.two_digits($_POST[$prefix.'_month']).'-'.two_digits(
        $_POST[$prefix.'_day']
    ).' '.two_digits($_POST[$prefix.'_hour']).':'.two_digits($_POST[$prefix.'_minute']).':00';
}
