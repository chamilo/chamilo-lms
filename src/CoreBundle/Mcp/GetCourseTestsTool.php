...
final readonly class GetCourseTestsTool
{
    ...
    /**
     * @return array{course_id: int, tests: list<array<string, mixed>>}
     */
    #[McpTool(
        name: 'get_course_tests',
        description: 'Read the title and configuration settings of tests (exercises) in a base course. Returns every test in the course, or a single one when testId or testTitle is provided. Available to the course\'s own teacher, and platform-wide to question managers and administrators.',
    )]
    public function getCourseTests(
        ...
        try {
            ...
            return [
                'course_id' => $courseId,
                'tests' => array_map(
                    fn ($quiz) => $this->testReader->normalizeTest($quiz, $course),
                    $quizzes,
                ),
            ];
