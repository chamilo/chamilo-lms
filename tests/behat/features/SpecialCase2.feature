Feature: Special admin settings flows — case 2
  ...
  Scenario: New user self-registration and first navigation
    ...
    # ---- REGISTRATION ----
    # Non-logged user lands on the homepage and clicks "Sign up"
    ...
    # Registration form is displayed ("Register" button visible)
    ...
    # Date of birth (hidden field populated by the date picker)
    ...
    # Target learning language: French
    ...
    # Sidebar is already visible after registration (no need to click chevron-up)
    # "Diagnosis management" is a PrimeVue panel with no direct href:
    # click the header to expand the submenu, then click "Diagnosis"
  ...
  Scenario: Admin creates tutors with language and assigns learner parkur01
    ...
    # ---- TUTOR CREATION (TCs) WITH ASSIGNED LANGUAGE ----
    # TODO: check available language extra fields on user_add.php for teachers
    ...
    # The select2 is AJAX-based (dynamic ID): open the dropdown via the span,
    # then type "parkur01" to trigger the search and click the result
    ...
    # ---- VIDEOCONFERENCE (BBB) ----
    # TODO: check that the Videoconference icon appears in the course menu
    # Prerequisites: BBB plugin enabled + host/salt configured + "Course tool" region assigned
  ...
  Scenario: Tuteur_fr visits student report and sends legal agreement
    ...
    # ---- BACK TO STUDENT FOLLOW-UP PAGE ----
    When I am on "/main/my_space/myStudents.php?student=67"
    And I wait very long for the page to be loaded
    And I zoom out to maximum
    Then I should not see an error
  ...
  Scenario: Teacher navigates sessions and course announcements
    ...
    # ---- SECOND EVENT: previous month → current month ----
    When I click the "span.mdi-calendar-plus" element
    And I wait very long for the page to be loaded
    Then I should see "Add event"
    ...
    # ---- THIRD EVENT: current month → next month ----
    When I click the "span.mdi-calendar-plus" element
    And I wait very long for the page to be loaded
    Then I should see "Add event"
    ...
    # ---- FOURTH EVENT: previous month → next month ----
    When I click the "span.mdi-calendar-plus" element
    And I wait very long for the page to be loaded
    Then I should see "Add event"
    ...
    # ---- BACK TO MESSAGES — check the first unchecked box ----
    When I go to "/resources/messages"
    And I wait very long for the page to be loaded
    Then I should see "Invitation Test Doodle"
    When I follow "Invitation Test Doodle"
