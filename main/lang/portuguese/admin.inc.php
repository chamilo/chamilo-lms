...
$PermissionsForNewFilesComment = "A capacidade de definir níveis de permissões a atribuir a todos os novos ficheiros criados permite aumentar a segurança contra ataques de hackers e o envio de conteúdo perigoso para o portal. A configuração pré-definida (0550) deve ser suficiente para man";
$langStudent = "Aluno";
$Guest = "Convidado";
...
$SplitUsersUploadDirectoryComment = "Em portais com uma utilização intensiva, onde existe um elevado número de utilizadores registados que submetem/enviam imagens, a pasta/directório de envio (main/upload/users) poderá conter demasiados ficheiros para poderem ser geridos pelo sistema de fich";
...
$DropboxAllowStudentToStudentComment = "Permitir que os utilizadores enviem documentos para outros utilizadores (peer 2 peer). Os utilizadores poderão utilizar esta funcionalidade para documentos menos relevantes (mp3, soluções de exercícios, ...). Desactivando esta funcionalidade, os utilizado";
...
$PermissionsForNewDirsComment = "A capacidade de definir os níveis de permissões a atribuir às novas pastas/diretórios criados, permite aumentar o nível de segurança contra ataques de hackers que poderão enviar conteúdos ilícitos e perigosos para o portal. A configuração pré-definida (07";
...
$ImageResizeComment = "As imagens dos utilizadores podem ser redimensionadas no envio se o PHP for compilado com a <a href=\\"http://php.net/manual/en/ref.image.php\\" target=\\"_blank\\">Biblioteca GD</a>. Se a <a href=\\"http://php.net/manual/en/ref.image.php\\" target=\\"_blank\\">B";
...
$langVisioconfDescription = "Chamilo LIVE é uma ferramenta de videoconferência que oferece: partilha de slides/apresentações, quadro branco/whiteboard para desenhar e escrever em cima de slides/apresentações, vídeo/áudio em duplex e chat. Requer o Adobe Flash Player 9.x e oferece 2 m";
$langPpt2lp = "Chamilo RAPID";
$langPpt2lpDescription = "Chamilo RAPID é uma ferramenta de autoria de Rapid Learning disponível no Chamilo Pro e no Chamilo Medical. Permite converter apresentações em Powerpoint e Openoffice para cursos compatíveis com a norma SCORM (SCORM-compliant). Após a conversão, com a fer";
...
$VersionCheckExplanation = "Para permitir a verificação automática da versão deverá registar o portal em chamilo.com. As informações obtidas ao clicar neste botão serão apenas para uso interno e apenas os dados agregados serão disponibilizados publicamente (número total de portais,";
...
$UseSessionModeComment = "As sessões de um curso, permitem uma forma diferente de lidar com os cursos que possuem um autor, um tutor e alunos. Cada instrutor disponibiliza um curso para um determinado grupo de alunos num determinado período de tempo a que chamamos *sessão do curso";
...
$PublicPagesComplyToWAIComment = "WAI (Web Accessibility Initiative) é uma iniciativa para tornar a web mais acessível. Seleccionando esta opção, as páginas públicas do Chamilo tornar-se-ão mais fáceis de aceder. Isto significa que alguns dos conteúdos nas páginas públicas do portal poder";
...
$Ajax_course_tracking_refresh_comment = "Esta opção é utilizada para calcular em tempo real o tempo que um utilizador despende num curso. O valor apresentado no campo é o intervalo em segundos em que o valor é actualizado. Para desactivar esta função, deixar \'0\' como valor pré-definido no camp";
...
$UploadExtensionsBlacklistComment = "A lista negra (Blacklist) é usada para filtrar as extensões dos ficheiros, removendo (ou renomeando) os ficheiros com extensões que fazem parte da lista negra abaixo. As extensões devem figurar sem o ponto (.) e separadas por pontos e vírgulas (;) tal com";
$UploadExtensionsWhitelistComment = "A lista branca (Whitelist) é usada para filtrar as extensões dos ficheiros, removendo (ou renomeando) qualquer ficheiro com extensões que *NÃO* fazem parte da lista branca abaixo. Considera-se geralmente como mais segura, mas também como uma aproximação d";
$UploadExtensionsSkip = "Comportamento do filtro (omitir/renomear)";
$UploadExtensionsSkipComment = "Se escolheu omitir, os ficheiros filtrados pela lista negra ou branca não serão carregados ou enviados pelo sistema. Se escolheu renomeá-los, a extensão será substituída por outra definida na configuração de substituição de extensões. No entanto, renomear";
...
$EphorusDescription = "Comece a utilizar o serviço anti-plágio Ephorus no Chamilo.<br /> <STRONG>Com o Ephorus e sem qualquer esforço adicional, impedirá o plágio através da Internet.</STRONG><br /> Pode utilizar o nosso serviço web para construir a sua própria integração ou ut";
...
$PlatformCharsetComment = "O conjunto de caracteres é o define a forma como alguns idiomas específicos podem ser exibidos no Chamilo. Se utilizar caracteres russos ou japoneses, por exemplo, poderá querer alterá-los. Para todos os caracteres em inglês, latim e ocidentais da europa,";
...
$NoReplyEmailAddressComment = "Este endereço de e-mail deverá ser utilizado quando é necessário enviar um e-mail a solicitar para não devolver a resposta ou responder de volta (no reply). Em geral, este endereço de e-mail deve ser configurado no servidor para ignorar ou eliminar qualqu";
...
$InstallDirAccessibleSecurityThreat = "A pasta/directório principal de instalação do Chamilo ainda se encontra acessível a qualquer utilizador da web. Este status/estado pode representar uma ameaça à segurança da sua instalação. Recomendamos que elimine esta pasta/directório ou então que alter";
...
$UserThemeSelectionComment = "Permitir que os utilizadores possam seleccionar o tema visual do seu perfil. Alterará o aspecto visual do Chamilo no perfil desse utilizador, mas deixará intacto o estilo de página pré-definido do portal. Se um curso específico ou uma sessão tiverem um te";
$AllowurlfopenIsSetToOff = "A configuração PHP \\"allow_url_fopen\\" está definida para \'off\'. Essa configuração evita que o mecanismo de registro funcione correctamente. Esta configuração pode ser alterada no ficheiro de configuração PHP (php.ini), ou na configuração do servidor vi";
...
$UserFieldsAddHelp = "Adicionar um campo de utilizador é muito fácil:<br /><br />- Seleccione uma palavra em letras minúsculas como elemento identificador,<br />- Seleccionar um tipo,<br />- Seleccione um texto que deverá aparecer ao utilizador (se utilizar uma palavra num idi";
$AllowCourseThemeTitle = "Permitir temas dos cursos";
$AllowCourseThemeComment = "Permite a utilização de temas gráficos nos cursos e a possibilidade de alterar o estilo de página utilizado por um outro diferente disponível no Chamilo. Quando um utilizador acede ao curso, o estilo de página do curso terá prioridade sobre o estilo de pá";
...
$ShowClosedCoursesComment = "Mostrar os cursos encerrados na página de login/acesso e na página de entrada/inicial do curso? Na página de entrada/inicial do portal aparecerá um ícone junto ao curso para se poder inscrever no mesmo. Isto só acontece na página de entrada/inicial do por";
$LDAPConnectionError = "Erro de conexão do LDAP";
$LDAP = "LDAP";
$LDAPEnableTitle = "Activar o LDAP";
$LDAPEnableComment = "Se tiver um servidor LDAP (Lightweight Directory Access Protocol), deverá de configurar as respectivas definições abaixo e modificar o ficheiro de configuração, tal como está descrito no \'Guia de Instalação\' e, em seguida, activá-lo. Irá permitir aos ut";
...
$LDAPFilledTutorFieldComment = "Será realizada uma verificação sobre o campo de contacto LDAP aquando da inserção de novos utilizadores. Se este campo não estiver vazio, o utilizador será considerado tutor e inserido no Chamilo como tal. Se desejar que todos os seus utilizadores sejam r";
...
$LDAPFilledTutorFieldValueComment = "Quando é feita uma seleccão no campo do tutor fornecido acima, este valor deverá estar dentro de um dos sub-elementos dos campos do tutor para que o utilizador possa ser considerado como um instrutor. Ao deixar este campo vazio, o campo só irá existir par";
...
$AllowSocialToolComment = "A ferramenta de colaboração/social networking (redes sociais) permite que os utilizadores criem/definam relações com outros utilizadores, criando grupos de amigos. Combinando esta ferramenta com a ferramenta interna de mensagens/chat, as comunicações com";
...
$AllowUseSubLanguageComment = "Ao activar esta opção, poderá definir variações para cada um dos termos do idioma utilizado no interface da plataforma, sob a forma de uma novo idioma com base neste e como uma extenção de um idioma já existente. Esta opção encontra-se disponível na secçã";
...
$CopyCourseFromSessionToSessionExplanation = "Ajuda para a cópia de cursos de uma sessão para outra sessão:<br></br>Para copiar cursos entre várias sessões, deverá em primeiro lugar, seleccionar um curso da lista da sessão de origem. Poderá copiar os conteúdos das ferramentas de descrição do curso, d";
...
$ItIsRecommendedInstallImagickExtension = "É recomendada a instalação da extensão imagick do PHP para se obter uma melhor definição na resolução das imagens, no momento da criação dos modelos de imagem em miniatura (thumbnails), caso contrário poderão ficar pouco perceptíveis. Se a extenção não es";
