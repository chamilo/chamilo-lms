    ...
    if ($type == 'link') {
        ...
        if (!api_valid_url($urllink, true)) { // A check against an absolute URL
            ...
            $result = Database :: query(
                "SELECT MAX(display_order) FROM  " . $tbl_link . " WHERE c_id = $course_id AND category_id = '" . intval(
            ...
            if ((api_get_setting(
                        'search_enabled'
                    ) == 'true') && $link_id && extension_loaded('xapian')
            ) {
...
function deletelinkcategory($type)
{
    ...
    if ($type == 'link') {
        ...
        if (isset ($_GET['id']) && $_GET['id'] == strval(intval($_GET['id']))) {
            $sql = "UPDATE $tbl_link SET on_homepage='0' WHERE c_id = $course_id AND id='" . intval(
                    $_GET['id']
                ) . "'";
            Database :: query($sql);
        }
    ...
    if ($type == 'category') {
        ...
        if (isset ($_GET['id']) && !empty ($_GET['id'])) {
            ...
            $sql = "DELETE FROM " . $tbl_categories . " WHERE c_id = $course_id AND id='" . intval(
                    $_GET['id']
                ) . "'";
            Database :: query($sql);
            $sql = "DELETE FROM " . $tbl_link . " WHERE c_id = $course_id AND category_id='" . intval(
                    $_GET['id']
                ) . "'";
...
function get_link_info($id)
{
    $tbl_link = Database :: get_course_table(TABLE_LINK);
    $course_id = api_get_course_int_id();
    $sql = "SELECT * FROM " . $tbl_link . " WHERE c_id = $course_id AND id='" . intval(
            $id
        ) . "' ";
...
function editlinkcategory($type)
{
    ...
    if ($type == 'link') {
        ...
        if ($_POST['submitLink']) {
            ...
            $_POST['description'] = trim(
                Security :: remove_XSS($_POST['description'])
            );
            ...
            if ($category_id != $_POST['selectcategory']) {
                $sql = "SELECT MAX(display_order) FROM " . $tbl_link . " WHERE c_id = $course_id AND category_id='" . intval(
                        $_POST['selectcategory']
                    ) . "'";
                $result = Database :: query($sql);
            ...
            if (api_get_setting('search_enabled') == 'true') {
                ...
                $link_description = Database :: escape_string(
                    $_POST['description']
                );
        ...
        while ($myrow = Database :: fetch_array($result)) {
            ...
            if (api_is_allowed_to_edit(null, true)) {
                $link_validator = '' . Display::url(
                        ...
                        '#',
                        array(
                            'onclick' => "check_url('" . $myrow['id'] . "', '" . addslashes(
                                $myrow['url']
                            ) . "');"
                        )
                    );
            ...
            if ($myrow['visibility'] == '1') {
                ...
                echo '<a href="link_goto.php?' . api_get_cidreq(
                    ) . '&amp;link_id=' . $myrow['id'] . '&amp;link_url=' . urlencode(
                        $myrow['url']
                    ) . '" target="_blank">
    			         <img src="../../main/img/link.gif" border="0" alt="' . get_lang(
                        'Link'
                    ) . '"/></a></td>
    			         <td width="80%" valign="top"><a href="link_goto.php?' . api_get_cidreq(
                    ) . '&amp;link_id=' . $myrow['id'] . '&amp;link_url=' . urlencode(
                        $myrow['url']
                    ) . '" target="' . $myrow['target'] . '">';
                ...
                if (api_is_allowed_to_edit(null, true)) {
                    echo '<tr class="' . $css_class . '">';
                    echo '<td align="center" valign="middle" width="15"><a href="link_goto.php?' . api_get_cidreq(
                        ) . '&amp;link_id=' . $myrow['id'] . "&amp;link_url=" . urlencode(
                            $myrow['url']
                    ...
                    echo '</td><td width="80%" valign="top"><a href="link_goto.php?', api_get_cidreq(
                    ), '&amp;link_id=', $myrow['id'], '&amp;link_url=', urlencode(
                        $myrow['url']
                    ), '" target="', $myrow['target'], '"  class="invisible">';
...
function showcategoryadmintools($categoryid)
{
    ...
    echo '<a href="' . api_get_self() . '?' . api_get_cidreq(
        ) . '&amp;sec_token=' . $token . '&amp;action=editcategory&amp;id=' . $categoryid . '&amp;urlview=' . $urlview . '" title=' . get_lang(
            'Modify'
        ) . '">' . Display :: return_icon(
            'edit.png',
            get_lang('Modify'),
    ...
    if ($catcounter != 1) {
        echo '<a href="' . api_get_self() . '?' . api_get_cidreq(
            ) . '&amp;sec_token=' . $token . '&amp;catmove=true&amp;up=', $categoryid, '&amp;urlview=' . $urlview . '" title="' . get_lang(
                'Up'
            ) . '">' . Display :: return_icon(
                'up.png',
                get_lang('Up'),
    ...
    echo '<a href="' . api_get_self() . '?' . api_get_cidreq(
        ) . '&amp;sec_token=' . $token . '&amp;action=deletecategory&amp;id=', $categoryid, "&amp;urlview=$urlview\" onclick=\"javascript: if(!confirm('" . get_lang(
            'CategoryDelconfirm'
...
function get_cat($catname)
{
    ...
    $result = Database :: query(
        "SELECT id FROM " . $tbl_categories . " WHERE c_id = $course_id AND category_title='" . Database::escape_string(
            $catname
        ) . "'"
    );
...
function put_link($url, $cat, $title, $description, $on_homepage, $hidden)
{
    ...
    $result = Database :: query(
        "SELECT id FROM $tbl_link WHERE c_id = $course_id AND " . $urleq . ' AND ' . $cateq
    );
...
function import_csvfile()
{
    ...
    if (is_uploaded_file(
        ...
        $listsep = strpos(
            $colnames = trim(fgets($myFile)),
            ','
        ) !== false ? ',' : (strpos($colnames, ';') !== false ? ';' : '');
        ...
        if ($listsep) {
            ...
            if (in_array('url', $columns) && in_array('title', $columns)) {
                ...
                if ($stats[0]) {
                    $catlinkstatus .= $stats[0] . ' ' . get_lang(
                            'CsvLinesFailed'
                        );
                }
                if ($stats[1]) {
                ...
                $catlinkstatus = get_lang(
                        'CsvFileNoURL'
                    ) . ($colnames ? get_lang(
                            'CsvFileLine1'
                        ) . htmlspecialchars(
                            substr($colnames, 0, 200)
                        ) . '...' : '');
            }
        } else {
            $catlinkstatus = get_lang('CsvFileNoSeps') . ($colnames ? get_lang(
                        'CsvFileLine1'
                    ) . htmlspecialchars(
                        substr($colnames, 0, 200)
                    ) . '...' : '');
