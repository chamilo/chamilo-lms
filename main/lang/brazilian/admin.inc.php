...
$PermissionsForNewFilesComment = "A capacidade para definir permissões e atribuí-las aos arquivos recém criados aumenta a segurança contra ataques de hackers que podem colocar material perigoso na plataforma. O valor pré-definido (0660) deveria ser suficiente para proporcionar ao servidor";
...
$SplitUsersUploadDirectoryComment = "Nos portais Chamilo com utilização intensa, onde estão inscritos um grande número de usuários que enviam as suas fotos pessoais, a diretórios que armazena essas imagens (main/upload/users) pode conter muitos arquivos para serem utilizados pelo sistema (fo";
...
$DropboxAllowStudentToStudentComment = "Permite aos estudantes enviarem arquivos para outros estudantes (ponto a ponto, P2P, troca). Os estudantes podem usar esta função para trocar documentos menos relevantes (mp3, soluções, ...). Se esta função for desabilitafs os estudantes poderão enviar do";
...
$PermissionsForNewDirsComment = "A capacidade para definir permissões e atribuí-las aos diretorios recém criadas aumenta a segurança contra ataques de hackers que podem colocar material perigoso na plataforma. O valor padrão (0770) deveria ser suficiente para proporcionar ao servidor um";
...
$langVisioconfDescription = "Chamilo Live Conferencing®  é uma ferramenta standard de videconferência que oferece: projeção de dispositivo, quadro para desenhar e escrever, possibilidade de conversas utilizando audio/video. Requer o player do Flash® e permite usar três modos: um para";
$langPpt2lp = "Oogie-Woogie Rota de Aprendizagem";
$langPpt2lpDescription = "Oogie-Woogie é uma ferramenta permite criar rotas de aprendizagem rapidamente disponíveis no Chamilo Pro e Chamilo Medicina. Permite-lhe converter apresentações Powerpoint e seus equivalentes do Openoffice SCORM. Após a conversão, o documento pode ser ger";
...
$VersionCheckExplanation = "Para ativar a verificação automática da versão do Chamilo é necessário realizar o registro no campus Chamilo.com. A informação obtida ao clicar neste botão é somente para uso interno e apenas os dados acrescentados estarão disponíveis publicamente (número";
...
$UseSessionModeComment = "A sessão, proporciona uma maneira diferente de lidar com os cursos, onde os cursos têm um autor, um tutor e alunos. Cada tutor dá um curso por um determinado período de tempo, chamado *sessão*, para cada conjunto de alunos para não misturar com outros gru";
...
$UploadExtensionsBlacklistComment = "A lista negra é usada para filtrar as extensões dos arquivos, removendo (ou renomeando) os arquivos com extensões que fazem parte da lista negra abaixo. As extensões devem figurar sem o ponto (.) e separadas por pontos e vírgulas (;) tal como se exemplifi";
$UploadExtensionsWhitelistComment = "A lista branca é usada para filtrar as extensões dos arquivos, removendo (ou renomeando) qualquer arquivo com extensões que *NÃO* fazem parte da lista branca abaixo. Considera-se geralmente como mais segura, mas também como uma aproximação de filtragem ma";
$UploadExtensionsSkip = "Comportamento do filtro (omitir/renomear)";
$UploadExtensionsSkipComment = "Se escolheu omitir, os arquivos filtrados pela lista negra ou branca não serão carregados ou enviados pelo sistema. Se escolheu renomeá-los, a extensão será substituída por outra definida na configuração de substituição de extensões. No entanto, renomear";
...
$EphorusDescription = "Comece a usar o serviço anti-plágio Ephorus no Chamilo.<br /> <STRONG>Com Ephorus, você pode impedir o plágio através da Internet sem qualquer esforço adicional.</STRONG><br /> Pode usar o nosso serviço web (standard) para construir a sua própria integraç";
...
$PlatformCharsetComment = "O tipo de caracteres é o que define idiomas específicos a serem mostrados no Chamilo. Se usar caracteres Russos ou Japoneses, por exemplo, você deve alterar esta opção. Para todos os caracteres em inglês, latinos e oeste da Europa, o tipo mais indicado é";
...
$InstallDirAccessibleSecurityThreat = "A pasta ou diretório de instalação main/install do Chamilo ainda está acessível aos usuários da Web. Esta situação poderá representar uma ameaça à segurança da sua instalação. Recomendamos que remova, apague ou altere as permissões do diretório, de modo q";
$GradebookActivation = "Ativação da ferramenta avaliação";
$GradebookActivationComment = "Ao ativar a ferramenta avalição, acrescentará a possibilidade de relacionar os objetivos de aprendizagem com os respectivos resultados contudo, se não necessita, é melhor não ativar, uma vez que tornará a interface desnecessariamente mais complexa. Deseja";
$UserTheme = "Tema (folha de estilos)";
$UserThemeSelection = "Seleção do tema pelo usuário";
$UserThemeSelectionComment = "Permitir que os usuários possam selecionar o seu próprio tema visual (no perfil). Isso muda o layout de Chamilo para eles, mas deixará intacto o estilo definido como padrão da plataforma. Se um curso ou sessão tiver um tema específico atribuído, terá prio";
$AllowurlfopenIsSetToOff = "O parâmetro PHP \\allow_url_fopen\\ está desativado. Isto impede que o mecanismo de registro funcione corretamente. Este parâmetro pode ser alterado no arquivo de configuração do PHP (php.ini) ou na configuração do Virtual Host do Apache, mediante a diret";
...
$UserFieldsAddHelp = "Adicionar um novo campo ao cadastro de usuário é muito fácil:<br />- basta escolher uma palavra como identificador usando letras minúsculas,<br />- selecionar um tipo,<br />- escolher o texto que deve aparecer ao usuário (se utilizar um nome já traduzido";
$AllowCourseThemeTitle = "Permitir temas personalizados para os cursos";
$AllowCourseThemeComment = "Permitir que os cursos possam ter um tema gráfico diferente, alterando a folha de estilo usada por uma das disponíveis em Chamilo. Quando um usuário entra num curso, a folha de estilo do curso tem preferência sobre a que esteja predefinida para a platafor";
...
$ShowClosedCoursesComment = "Exibir os cursos encerrados na página de login e na página inicial? Na página inicial um ícone para uma inscrição rápida aparecerá próximo ao curso. O ícone apenas aparecerá na página inicial quando o usuário estiver logado e o usuário ainda não for inscr";
$LDAPConnectionError = "Erro de conexão LDAP";
$LDAP = "LDAP";
$LDAPEnableTitle = "Ativar LDAP";
$LDAPEnableComment = "Caso você trabalhe com um servidor LDAP deverá configurar as opções abaixo e modificar seu arquivo e configurações conforme descrito no manual de instalação antes de ativá-lo. Isto permitirá que os usuários usem seus logins LDAP para autenticação. Caso vo";
...
$LDAPFilledTutorFieldComment = "Uma verificação será realizada sobre o campo de contato LDAP quando da inserção de novos usuários. Se este campo não estiver vazio, o usuários será considerado tutor e inserido no Chamilo como tal. Se desejar que todos os seus usuários sejam reconhecidos";
...
$VisioUseRtmptComment = "O protocolo RTMTP permite o acesso à videoconferência através de um computador protegido por uma firewall, reorientado a comunicação para porta 80. Esta solução torna mais lenta a difusão de vídeo e áudio e, por essa razão, não é recomendada se não for es";
...
$LDAPFilledTutorFieldValueComment = "Quando se realiza uma verificação no campo tutor que aparece acima, para que o usuário seja considerado professor, o valor que se lhe atribui deve ser um dos sub-elementos do campo professor. Se deixa este campo em branco, a única condição para que seja c";
...
$AllowSocialToolComment = "A ferramenta de redes sociais permite que se criar relações com outros usuários, criando grupos de amigos. Combinando esta ferramenta com a ferramenta interna de mensagens/chat, as comunicações com esses grupos de amigos tornam-se mais estreitas, dentro d";
...
$AllowUseSubLanguageComment = "Ao ativar esta opção, você será capaz de definir as variações para cada um dos termos utilizados na interface de idioma da plataforma, sob a forma de uma novo idioma baseada na extensão e de uma idioma existente. Você encontrará essa opção de idioma seção";
