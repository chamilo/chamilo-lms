...
if ($editFile && $isPlatformAdmin) {
    $originalContent = Container::$container->get(FileHelper::class)->read($file);
    ...
    $content = '';
    if (Container::$container->get(FileHelper::class)->exists($maintenanceHtml)) {
        $content = Container::$container->get(FileHelper::class)->read($maintenanceHtml);
    }
    if (empty($content)) {
...
';
    ...
    if ($form->validate()) {
        ...
        Container::$container->get(FileHelper::class)->write($file, $newFileContent);
        ...
        if (in_array($httpCode, $statusOkList)) {
            $result = Container::$container->get(FileHelper::class)->write($maintenanceHtml, $content);
            if (false === $result) {
                Display::addFlash(
                    Display::return_message(
            ...
            Container::$container->get(FileHelper::class)->write($file, $originalContent);
        }

        if (false == $active) {
            $message = $plugin->get_lang('MaintenanceModeIsOff');
            $contentNoBlock = str_replace("\x0D", '', $contentNoBlock);
            Container::$container->get(FileHelper::class)->write($file, $contentNoBlock);
        } else {
