...
function exportNow() {
  ...
  if (cidNum) params.append("cid", String(cidNum))
  if (sidNum) params.append("sid", String(sidNum))
  params.append("items", Array.from(selected.value).join(","))
...
watch(() => props.show, (open) => { if (open) fetchExportables() })
onMounted(() => { if (props.show) fetchExportables() })
</script>
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4">
      <div class="w-full max-w-xl bg-white rounded-2xl shadow-xl border border-gray-25">
        <header class="px-5 py-4 border-b border-gray-15 flex items-center justify-between">
          ...
          <button class="w-8 h-8 grid place-content-center rounded-lg hover:bg-gray-15" @click="onClose" :aria-label="t('Close')">
            <i class="mdi mdi-close text-xl" />
          </button>
        </header>

        <section class="p-5 max-h-[60vh] overflow-auto">
          <div v-if="loading" class="animate-pulse text-gray-50">{{ t("Loading...") }}</div>
          ...
          <div v-else>
            ...
            <div v-else class="space-y-3">
              <div class="flex items-center justify-between">
                ...
                <button class="px-3 py-1.5 rounded-lg border border-gray-25 hover:bg-gray-15"
                        @click="toggleAll">
              ...
              <ul class="divide-y divide-gray-15 rounded-xl border border-gray-15">
                <li v-for="it in items" :key="it.id" class="px-3 py-2 flex items-center gap-3">
                  <input
                    type="checkbox"
                    class="w-4 h-4"
        ...
        <footer class="px-5 py-4 border-t border-gray-15 flex items-center justify-end gap-2">
          <button class="px-4 py-2 rounded-xl border border-gray-25 hover:bg-gray-15" @click="onClose">
            {{ t("Cancel") }}
          </button>
          <button
