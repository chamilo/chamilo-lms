...
class Plugin
{
    ...
    /**
     * Normalize default values for plugin settings forms.
     */
    private function getNormalizedDefaultValue(string $type, mixed $value, ?array $options = null): mixed
    {
        if (null !== $value && '' !== $value) {
            return $value;
        }
        ...
        if ('select' === $type && is_array($options)) {
            ...
            foreach (array_keys($options) as $key) {
                $normalizedKeys[strtolower((string) $key)] = $key;
            }

            foreach (['false', '0', 'no'] as $candidate) {
                if (isset($normalizedKeys[$candidate])) {
    ...
    public function getSettingsForm()
    {
        ...
        foreach ($fields as $name => $type) {
            ...
            if (is_array($type) && isset($type['type'])) {
                ...
                if ('select' === $fieldType) {
                    if (!empty($type['options']) && !empty($type['translate_options'])) {
            ...
            $value = $this->get($name);
            $value = $this->getNormalizedDefaultValue((string) $fieldType, $value, $options);
            $defaults[$name] = $value;
            $type = $fieldType ?: 'text';
