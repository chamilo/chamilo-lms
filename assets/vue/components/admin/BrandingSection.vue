      ...
      <section class="rounded-xl border border-gray-100 bg-gray-20 p-4">
        ...
        <div class="flex items-center gap-4">
          <img
            ...
            alt="Header logo"
            class="h-auto max-h-[50px] max-w-[190px] w-auto object-contain"
            @error="onImgError"
          />
          <span
            v-if="!hasHeaderCustom"
      ...
      <section class="grid grid-cols-1 gap-3">
        ...
        <div>
          ...
          <input
            type="file"
            accept="image/png"
            @change="onPick($event, 'headerPng', { maxW: 190, maxH: 60 })"
          />
        </div>
      </section>
      ...
      <section class="grid grid-cols-1 gap-3">
        ...
        <div>
          ...
          <input
            type="file"
            accept="image/png"
            @change="onPick($event, 'emailPng', { recommendW: 540 })"
          />
        </div>
      </section>
  ...
  if (key === "headerPng" || key === "emailPng") {
    ...
    img.onload = () => {
      if (key === "headerPng" && (img.width > 190 || img.height > 60)) {
        alert(t("Header PNG must be ≤ {0}px.", ["190×60"]))
        e.target.value = ""
        return
      }
...
function clearLocal(which) {
  if (which === "header") {
    files.value.headerSvg = null
    files.value.headerPng = null
  }
  if (which === "email") {
    files.value.emailSvg = null
    files.value.emailPng = null
  }
}
