...
class TestPENSPackageHandler extends UnitTestCase
{
    ...
    {
        $object = new MyPackageHandler();
        $object->setSupportedPackageTypes(['aicc-pkg', 'testing']);
        $this->assertEqual($object->getSupportedPackageTypes(), ['aicc-pkg']);
    }

    public function testSupportedPackageTypesValid()
    {
        $object = new MyPackageHandler();
        $object->setSupportedPackageTypes(['aicc-pkg', 'lms-qti']);
        $this->assertEqual($object->getSupportedPackageTypes(), ['aicc-pkg', 'lms-qti']);
    }
    ...
    {
        $object = new MyPackageHandler();
        $object->setSupportedPackageFormats(['zip', 'testing']);
        $this->assertEqual($object->getSupportedPackageFormats(), ['zip']);
    }

    public function testSupportedPackageFormatsValid()
    {
        $object = new MyPackageHandler();
        $object->setSupportedPackageFormats(['zip', 'jar']);
        $this->assertEqual($object->getSupportedPackageFormats(), ['zip', 'jar']);
    }
}
