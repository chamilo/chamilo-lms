...
{% if sessions_are_included %}
    <ul class="nav nav-tabs buy-courses-tabs" role="tablist">
        <li role="presentation" class="{{ courses ? 'active' : '' }}">
            <a href="{{ url('index') ~ 'plugin/BuyCourses/src/subscriptions_courses.php' }}" >
                {{ 'Courses'|get_lang }}
            </a>
        </li>
        <li role="presentation" class="{{ sessions ? 'active' : '' }}">
            <a href="{{ url('index') ~ 'plugin/BuyCourses/src/subscriptions_sessions.php' }}" >
                {{ 'Sessions'|get_lang }}</a>
        </li>
    </ul>
{% endif %}
            ...
            <table id="courses_table" class="table table-striped table-hover">
                ...
                {% for item in courses %}
                    <tr data-item="{{ item.id }}" data-type="course">
                        ...
                        <td class="text-right">
                            {% if item.buyCourseData %}
                                <a href="{{ url('index') ~ 'plugin/BuyCourses/src/configure_subscription.php?' ~ {'id': item.id, 'type':product_type_course}|url_encode }}"
                                class="btn btn--info btn--sm">
                                    <em class="fa fa-wrench fa-fw"></em> {{ 'Configure'|get_lang }}
                                </a>
                            {% else %}
                                <a href="{{ url('index') ~ 'plugin/BuyCourses/src/subscription_add.php?' ~ {'id': item.id, 'type':product_type_course}|url_encode }}"
                                class="btn btn--info btn--sm">
                                    <em class="fa fa-wrench fa-fw"></em> {{ 'Configure'|get_lang }}
                                </a>
                            {% endif %}
                ...
                <table id="session_table" class="table">
                    ...
                    {% for item in sessions %}
                        <tr data-item="{{ item.id }}" data-type="session">
                            <td>
                                <a href="{{ url('index') ~ 'main/session/index.php?' ~ {'session_id': item.id}|url_encode() }}">{{ item.name }}</a>
                            </td>
                            <td class="text-center">
                            ...
                            <td class="text-right">
                                {% if item.buyCourseData %}
                                    <a href="{{ url('index') ~ 'plugin/BuyCourses/src/configure_subscription.php?' ~ {'id': item.id, 'type':product_type_session}|url_encode }}"
                                    class="btn btn--info btn--sm">
                                        <em class="fa fa-wrench fa-fw"></em> {{ 'Configure'|get_lang }}
                                    </a>
                                {% else %}
                                    <a href="{{ url('index') ~ 'plugin/BuyCourses/src/subscription_add.php?' ~ {'id': item.id, 'type':product_type_session}|url_encode }}"
                                    class="btn btn--info btn--sm">
                                        <em class="fa fa-wrench fa-fw"></em> {{ 'Configure'|get_lang }}
                                    </a>
                                {% endif %} 
