...
if (!empty($error)) {
    ...
    $currentQuestionJsVariable = $current_question;
    ...
    echo '<script>
            ...
            function saveQuestionList(question_list) {
                ...
                var url = "";
                if ('.$reminder.' == 1) {
                    url = "'.$urlMainExercise.'exercise_reminder.php?'.$params.'&num='.$currentQuestionJsVariable.'";
                } else if ('.$reminder.' == 2 ) {
                    url = "'.$urlMainExercise.'exercise_submit.php?'.$params.'&num='.$currentQuestionJsVariable.'&remind_question_id='.$remind_question_id.'&reminder=2";
                } else {
    ...
    echo '<form id="exercise_form" method="post" action="'.api_get_path(WEB_PUBLIC_PATH).'main/exercice/exercise_submit.php?'.api_get_cidreq().'&autocomplete=off&gradebook='.$gradebook."&exerciseId=".$exerciseId .'" name="frm_exercise" '.$onsubmit.'>
     ...
     <input type="hidden" name="num" 					value="'.$currentQuestionJsVariable.'" id="num_current_id" />
