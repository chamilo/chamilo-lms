...
use Chamilo\CoreBundle\ApiResource\Exercise\ExerciseRuntimeAttemptRecalculate;
...
/**
 * Recalculates one learner attempt using the migrated scoring service.
 *
 * @implements ProcessorInterface<ExerciseRuntimeAttemptRecalculate, ExerciseRuntimeAttemptRecalculate>
 */
final readonly class ExerciseRuntimeAttemptRecalculateProcessor implements ProcessorInterface
{
     ...
     */
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): ExerciseRuntimeAttemptRecalculate
    {
        if (!$data instanceof ExerciseRuntimeAttemptRecalculate) {
            throw new BadRequestHttpException('Invalid recalculation request.');
        }
        ...
        $result = $this->scoringService->recalculateAttempt($attempt, $quiz);
        $this->entityManager->flush();

        $response = new ExerciseRuntimeAttemptRecalculate();
