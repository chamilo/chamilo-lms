...
function fill_Db_course($courseDbName, $courseRepository, $language)
{
	...
	*/
	...
	if(api_service_is_active('visio')){
...
function readPropertiesInArchive($archive, $isCompressed = TRUE)
{
	...
	tempnam() crï¿½ un fichier temporaire unique dans le dossier dir. Si le dossier n'existe pas, tempnam() va gï¿½ï¿½er un nom de fichier dans le dossier temporaire du systï¿½e.
	Avant PHP 4.0.6, le comportement de tempnam() dï¿½endait de l'OS sous-jacent. Sous Windows, la variable d'environnement TMP remplace le paramï¿½re dir; sous Linux, la variable d'environnement TMPDIR a la prioritï¿½ tandis que pour les OS en systï¿½e V R4, le paramï¿½re dir sera toujours utilisï¿½ si le dossier qu'il reprï¿½ente existe. Consultez votre documentation pour plus de dï¿½ails.
	tempnam() retourne le nom du fichier temporaire, ou la chaï¿½e NULL en cas d'ï¿½hec.
	*/
