    ...
    if (!empty($needle) && !empty($type)) {
        ...
        switch ($type) {
            ...
            case 'multiple':
                ...
                ) ? 'firstname' : 'lastname').' LIKE "'.$needle.'%" AND user.status<>'.DRH.' AND user.status<>6 '.$cond_user_id.
                    $order_clause;
                break;
            case 'any_session':
...
$link_add_group = '<a href="usergroups.php">'.Display::return_icon(
    'multiple.gif',
    get_lang('RegistrationByUsersGroups')
).get_lang('RegistrationByUsersGroups').'</a>';
?>
<div class="actions">
        ...
        <div class="span5">
            ...
            <div id="content_source">
                ...
                if (!($add_type == 'multiple')) {
                    ...
                    <div id="ajax_list_users_multiple">
                        <select id="origin_users" name="nosessionUsersList[]" multiple="multiple" size="15"
                            ...
                            foreach ($nosessionUsersList as $uid => $enreg) {
                                ?>
                                <option value="<?php echo $uid; ?>" <?php if (in_array(
                                    $uid,
                                    $UserList
                                )
                                ) {
                                        echo 'selected="selected"';
                                    } ?>><?php echo api_get_person_name(
                                    $enreg['fn'],
                                    $enreg['ln']
                                ).' ('.$enreg['un'].')'; ?></option>
                                <?php
                    ...
                    <input type="checkbox" onchange="checked_in_no_session(this.checked);" name="user_with_any_session"
                           id="user_with_any_session_id">
        ...
        <div class="span5">
            ...
            <select id="destination_users" name="sessionUsersList[]" multiple="multiple" size="15" class="span5">
                ...
                foreach ($sessionUsersList as $enreg) {
                    ?>
                    <option value="<?php echo $enreg['user_id']; ?>"><?php echo api_get_person_name(
                        $enreg['firstname'],
                        $enreg['lastname']
