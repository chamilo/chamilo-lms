...
#
# Navigates to the course description tool via its legacy compat URL
# (public/main/course_description/index.php, a plain course-context redirect
# to the real SPA route) instead of "I follow "Course description"":
# confirmed live that setting a course's own language away from the admin's
# session language (the whole point of these scenarios) also renders that
# course's tool menu in ITS OWN language, so the English link text is never
# on the page at all.
