...
$langYourDokeosUses = "Ваша Chamilo установка используется сейчас";
...
$PermissionsForNewFilesComment = "Возможность определять права настроек для каждого нового создаваемого файла позволяет Вам повышать уровень безопасности против атак хакеров, которые могут загружать файлы сомнительного содержания на Ваш портал. Стандартной настройки (0550) должно быть дос";
$langStudent = "Студент";
$Guest = "Гость";
...
$Properties = "Свойства";
$DokeosConfigSettings = "Настройки Chamilo";
...
$SplitUsersUploadDirectoryComment = "На каждом часто посещаемом портале, где много пользователей регистрируется и посылает свои фото, директория пересылки (главная/пересылка/пользователи/) может содержать слишком много файлов для управления системными файлами (например, на сервере Debain был";
...
$DropboxAllowStudentToStudentComment = "Разрешить студентам посылать документы другим студентам (равный равному, обмен между равными). Студенты могут использовать эту функцию и для менее значимых документов (музыка, видео...). Если Вы эту функцию отключите, то студент сможет посылать документы";
...
$PermissionsForNewDirsComment = "Возможность определять набор прав при создании каждой новой директории (папки) позволяет Вам повысить безопасность и защититься от аттак хакеров, которые могут скачивать документы сомнительного содержания на Ваш портал. Настроек по умолчанию (0770) должно";
...
$DokeosHomepage = "Домашняя страница Chamilo (Докеос)";
...
$langVisioconfDescription = "Докеос конференция  в прямом эфире® - это стандартный инструмент видеоконференции, который предлагает: показ слайдов, белая доска для рисования или записей фломастерами, спаренное аудио/видео, чат. Она требует только Flash® плеер и позволяет использовать";
...
$VersionCheckExplanation = "Для того чтобы, включать автоматическую проверку версии Вы должны зарегистрировать свой портал на chamilo.com. Информация, получаемая с помощью щелчка по этой кнопке, только для внутреннего пользования и только содержит данные открытого доступа (общее кол";
...
$UploadExtensionsBlacklistComment = "Черный список используется для фильтрации расширений (типов) файлов с помощью удаления (или переименования) любого файла, расширение которого имеется в черном списке ниже. Расширение должно быть без точки (.), разделяться точкой с запятой (;) как следующи";
$UploadExtensionsWhitelistComment = "Белый список используется для фильтрации расширений (типов) файлов с помощью удаления (или переименования) любого файла, расширение которого \\"НЕ\\" имеется в белом списке ниже. Это считается наиболее безопасным способом фильтрации, но в тоже время и наибо";
$UploadExtensionsSkip = "Поведение фильтрования (пропустить/переименовать)";
$UploadExtensionsSkipComment = "Если вы выбирите опцию пропустить, файлы, фильтруемые через черный или белый список не будут загружены в систему. Если Вы выбирите опцию переименовать их, их расширения будут заменены на одно из расширений из настроек замены. Знайте, что переименование фа";
...
$EphorusDescription = "Начать использовать антиплагиатную службу Ephorus в Докеос.<br /> <STRONG>С Ephorus Вы предотвратите интернет плагиат без дополнительных усилий.</STRONG><br /> Вы можете использовать нашу уникальную открытую стандартную интернет службу для постройки своей";
$EphorusLeadersInAntiPlagiarism = "<STRONG>Лидеры в";
...
$PlatformCharsetComment = "Набор символов - кодировка, которая настраивает отображение определенного языка в Докеос. Если Вы используете Русский или Японский наборы, например, Вы можете захотеть их сменить. Для всех английских, латинских и западно-европейских наборов по умолчанию у";
...
$NoReplyEmailAddressComment = "Этот эл. адрес предназначен для использования в случае, когда в посланном письме оговаривалось, что на этот адрес не следует посылать ответ. В общем, этот эл. адрес должен быть настроен на вашем сервере так, чтобы не принимать во внимание/игнорировать люб";
...
$InstallDirAccessibleSecurityThreat = "Главная/установленная директория (папка) Вашей системы Chamilo до сих пор остается доступной для пользователей сети интернета. Это может послужить угрозой безопасности для ваших установок. Мы рекомендуем убрать эту директорию (папку) или изменить ее права";
...
$UserThemeSelectionComment = "Позволяет пользователям выбирать их собственную визуальную тему в своем профиле. Это изменит вид Докеос для них, но дизайн портала по умолчанию останется нетронутым. Если конкретному курсу или сессии присваивается конкретная тема, это дает ей преимущество";
$AllowurlfopenIsSetToOff = "PHP настройка \\"allow_url_fopen\\" означает «выключено». Это не позволяет механизму регистрации работать правильно. Эта настройка может быть изменена в Вашем конфигурационном PHP файле (php.ini) или в конфигурациях виртуального хоста Apache, используя php_";
