...
class Blog
{
    ...
    public static function create_blog($title, $subtitle)
    {
        ...
        $sql = 'SELECT COUNT(*) as count FROM '.$tbl_blogs.'  WHERE c_id = '.$course_id.' AND blog_name="'.Database::escape_string(
            $title
        ).'" AND blog_subtitle="'.Database::escape_string($subtitle).'";';
    ...
    public static function edit_blog($blog_id, $title, $subtitle)
    {
        ...
        // Update the blog
        $sql = "UPDATE $tbl_blogs SET blog_name = '".Database::escape_string(
            $title
        ...
        api_item_property_update(
            api_get_course_info(),
            TOOL_BLOGS,
            Database::escape_string($blog_id),
            'BlogUpdated',
            api_get_user_id()
        );

        // Update course homepage link
        $sql = "UPDATE $tbl_tool SET name = '".Database::escape_string(
            $title
        )."' WHERE c_id = $course_id AND link = 'blog/blog.php?blog_id=".Database::escape_string(
            (int)$blog_id
        )."' LIMIT 1";
        Database::query($sql);
    }
    ...
    public static function display_blog_posts($blog_id, $filter = '1=1', $max_number_of_posts = 20)
    {
        ...
        if (Database::num_rows($result) > 0) {
            ...
            while ($blog_post = Database::fetch_array($result)) {
                ...
                if ($words >= $limit) {
                    $readMoreLink = ' <div class="link" onclick="document.getElementById(\'blogpost_text_'.$blog_post_id.'\').style.display=\'block\'; document.getElementById(\'blogpost_introduction_'.$blog_post_id.'\').style.display=\'none\'">'.get_lang(
                        'ReadMore'
                    ).'</div>';
                ...
                echo '<span class="blogpost_title"><a href="blog.php?action=view_post&amp;blog_id='.$blog_id.'&amp;post_id='.$blog_post['post_id'].'#add_comment" title="'.get_lang(
                    'ReadPost'
                ).'" >'.stripslashes($blog_post['title']).'</a></span>';
                echo '<span class="blogpost_date"><a href="blog.php?action=view_post&amp;blog_id='.$blog_id.'&amp;post_id='.$blog_post['post_id'].'#add_comment" title="'.get_lang(
                    'ReadPost'
    ...
    public static function get_threaded_comments($current = 0, $current_level = 0, $blog_id, $post_id, $task_id = 0)
    {
        ...
        while ($comment = Database::fetch_array($result)) {
            ...
            echo '<span class="blogpost_comment_title"><a href="#add_comment" onclick="document.getElementById(\'comment_parent_id\').value=\''.$comment['comment_id'].'\'; document.getElementById(\'comment_title\').value=\'Re: '.addslashes(
                $comment['title']
            ).'\'" title="'.get_lang('ReplyToThisComment').'" >'.stripslashes($comment['title']).'</a></span>';
            ...
            echo '<span class="blogpost_comment_info">'.get_lang('Author').': '.Display::tag(
                'span',
                api_get_person_name($comment['firstname'], $comment['lastname']),
                array('title' => $username)
            ).' - '.get_lang('Rating').': '.Blog::display_rating(
                'comment',
                $blog_id,
                $comment['comment_id']
            ).$rating_select.'</span>';
    ...
    public static function display_form_new_post($blog_id)
    {
        if (api_is_allowed('BLOG_'.$blog_id, 'article_add')) {
            ...
            $url = api_get_path(WEB_PUBLIC_PATH).'main/blog/blog.php?action=new_post&blog_id='.$blog_id;
    ...
    public static function display_form_edit_post($blog_id, $post_id)
    {
        ...
        $form = new FormValidator('edit_post', 'post', $url, null, array('enctype' => 'multipart/form-data'));
        $form->addElement('header', get_lang('EditPost'));
        $form->addElement('hidden', 'edit_post_submit', 'true');
        $form->addElement('hidden', 'post_id', intval($_GET['post_id']));
    ...
    public static function display_form_user_subscribe($blog_id)
    {
        ...
        foreach ($student_list as $key => $user) {
            ...
            if (!in_array($user['user_id'], $blog_member_ids)) {
                ...
                $row = array();
                $row[] = '<input type="checkbox" name="user[]" value="'.$a_infosUser['user_id'].'" '.(($selectAll == "subscribe") ? ' checked="checked" ' : '').'/>';
    ...
    {
        $url = api_get_path(WEB_PUBLIC_PATH).'main/blog/blog.php?action=view_post&blog_id='.intval($blog_id).'&post_id='.intval($post_id);
        $form = new FormValidator('add_post', 'post', $url, null, array('enctype' => 'multipart/form-data'));
        $form->addElement('header', isset($_GET['task_id']) ? get_lang('ExecuteThisTask') : get_lang('AddNewComment'));
        $form->addElement('text', 'comment_title', get_lang('Title'));

        $form->addElement('html_editor', 'comment_text');
