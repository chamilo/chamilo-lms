<template>
  <div>
    ...
    >
      ...
      <Column :header="t('Type')" headerStyle="width:4rem" bodyClass="text-center">
        <template #body="{ data }">
      ...
      <Column :header="t('Received Files')">
        <template #body="{ data }">
          ...
          <template v-if="data.kind==='folder'">
            <a class="font-semibold cursor-pointer hover:underline" @click="enterCat(data.catId)">
              {{ data.title }}
            </a>
          </template>
          ...
          <template v-else>
            <a
              ...
              rel="noopener"
            >
              <i :class="chamiloIconToClass['download']" class="text-base leading-none"></i>
            </a>
...
async function onMoved() { await load() }
