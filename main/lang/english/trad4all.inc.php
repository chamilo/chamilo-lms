...
$PersonalDataMainQuestion = "What do we consider personal data?";
$PersonalDataCollectionQuestion = "Why do we collect this data?";
$PersonalDataRecordingQuestion = "Where do we record the data? (in which system, hosted where, accessible by which company that handles the servers, etc.)";
$PersonalDataOrganizationQuestion = "How is the data structured? To what extent does the organization of the data protect your security? ... Do you have established processes that ensure that a hacker will not be able to collect all the personal data on the first attempt? Are they encrypted?";
$PersonalDataStructureQuestion = "How is the data structured in this software? (one tab for all the data, it has some categorization / label, ...)";
$PersonalDataConservationQuestion = "How long do we save the data? Is there data that expires even though the account is still active? How much time do we keep the data after the last use of the system?";
$PersonalDataAdaptationQuestion = "What changes can we make to the data? What changes can be made to the data without affecting the service?";
$PersonalDataExtractionQuestion = "What do we extract data for (towards other internal processes) and which data is it?";
$PersonalDataQueriesQuestion = "Who can consult the personal data? For what purpose?";
$PersonalDataUseQuestion = "How and for what can we use the personal data?";
$PersonalDataCommunicationQuestion = "With whom can we share them? In what opportunities? Through what processes (safe or not)?";
$PersonalDataInterconnectionQuestion = "Do we have another system with which Chamilo interacts? What is the interconnection process? What are the data pieces exchanged and in what way?";
$PersonalDataLimitationQuestion = "What are the limits that we will always respect when using personal data? How far can we go?";
$PersonalDataDeletionQuestion = "After how long do we erase the data? (event, last use, contract validity, etc.) What are the elimination processes?";
$PersonalDataDestructionQuestion = "What happens if the data is destroyed as a result of a technical failure? (unauthorized deletion or loss of material, for example)";
$PersonalDataProfilingQuestion = "For what purpose do we process personal data? Do we use it to filter the access users have to certain parts of our application? (negative or positive discrimination)";
?>