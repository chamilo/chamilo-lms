...
/**
 * Fresh re-run of Version20250926174000's fixtures-upsert logic, so that any
 * setting added to SettingsCurrentFixtures.php since then (here: the file
 * integrity monitoring settings, "security.file_integrity_check_enabled" and
 * "security.file_integrity_check_notify_admins") gets its settings/settings_current
 * row created on existing installs — a fresh install already gets every
 * fixture-defined setting for free via SettingsManager::installSchemas(), but
 * an upgrade only runs migrations, not fixtures.
 */
final class Version20260721125301 extends AbstractMigrationChamilo
{
