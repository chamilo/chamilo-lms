...
use ChamiloLMS\CoreBundle\Component\Auth\LoginSuccessHandler;
use ChamiloLMS\CoreBundle\Component\Auth\LogoutSuccessHandler;
use ChamiloLMS\CoreBundle\Component\Auth\LoginListener;
use ChamiloLMS\CoreBundle\Component\Editor\Connector;
use ChamiloLMS\CoreBundle\Component\Validator\ConstraintValidatorFactory;
use ChamiloLMS\CoreBundle\Component\Mail\MailGenerator;
...
/*
use ChamiloLMS\CoreBundle\Component\Auth\CourseVoter;
use ChamiloLMS\CoreBundle\Component\Auth\CourseAccessDecisionManager;
...
class ChamiloServiceProvider implements ServiceProviderInterface
{
    public function register(Application $app)
    {
        ...
        $app['html_editor'] = $app->share(function($app) {
            $editor = new ChamiloLMS\CoreBundle\Component\Editor\CkEditor\CkEditor(
                $app['translator'],
                $app['url_generator'],
                $app['template'],
                $app['course']
            ...
            return $editor;
            /*return new ChamiloLMS\CoreBundle\Component\Editor\TinyMce\TinyMce(
        ...
        $app['course'] = $app->share(function () use ($app) {
            ...
            if (!empty($courseCode)) {
                ...
                return $app['orm.em']->getRepository('ChamiloLMS\CoreBundle\Entity\Course')->findOneByCode($courseCode);
        ...
        $app['course_session'] = $app->share(function () use ($app) {
            ...
            if (!empty($sessionId)) {
                return $app['orm.em']->getRepository('ChamiloLMS\CoreBundle\Entity\Session')->findOneById($sessionId);
//                $app['template']->assign('course_session', $courseSession);
        ...
        $app['exporter'] = $app->share(function () use ($app) {
            return new ChamiloLMS\CoreBundle\Framework\Exporter();
        });
...
foreach ($controllers as $name => $controllerInfo) {
    $class = 'ChamiloLMS\CoreBundle\Controller\\' . $controllerInfo[0];
    $app[$name] = $app->share(
...
/*
$app->register(
    new ChamiloLMS\CoreBundle\Provider\BootstrapSilexProvider(),
    array(
