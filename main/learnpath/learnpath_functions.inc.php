...
* @author		Yannick Warnier <yannick.warnier@dokeos.com>, multi-level learnpath behaviour
* @access  public
...
function deleteitem($id)
{
	...
	$sql = "SELECT display_order, chapter_id FROM $tbl_learnpath_item WHERE id=$id";
	...
	$sql = "UPDATE $tbl_learnpath_item SET display_order = display_order-1 WHERE display_order > $display_order AND chapter_id = $chapter_id";
	$result = api_sql_query($sql, __FILE__, __LINE__);
	$sql = "UPDATE $tbl_learnpath_chapter SET display_order = display_order-1 WHERE display_order > $display_order AND parent_chapter_id = $chapter_id";
	$result = api_sql_query($sql, __FILE__, __LINE__);
	//return
	return true;
}
 ...
 **/
function deletemodule($chapter_id)
{
	global $learnpath_id;
	$tbl_learnpath_item = Database :: get_course_table(LEARNPATH_ITEM_TABLE);
	$tbl_learnpath_chapter = Database :: get_course_table(LEARNPATH_CHAPTER_TABLE);
	...
	{
		if ($row['parent_chapter_id'] == $chapter_id)
		{
	...
	$sql = "SELECT display_order, parent_chapter_id FROM $tbl_learnpath_chapter WHERE id=$chapter_id and learnpath_id=$learnpath_id";
	$result = api_sql_query($sql, __FILE__, __LINE__);
	if (mysql_num_rows($result) == 0)
	{
		return false;
	...
	$sql = "DELETE FROM $tbl_learnpath_chapter WHERE (id=$chapter_id and learnpath_id=$learnpath_id)";
	$result = api_sql_query($sql, __FILE__, __LINE__);
	//delete items from that chapter
	$sql2 = "DELETE FROM $tbl_learnpath_item WHERE chapter_id=$chapter_id";
	$result = api_sql_query($sql2, __FILE__, __LINE__);

	//update all other chapters accordingly
	$sql = "UPDATE $tbl_learnpath_item SET display_order = display_order-1 WHERE display_order > $display_order AND chapter_id = $parent_id";
	$result = api_sql_query($sql, __FILE__, __LINE__);
	$sql = "UPDATE $tbl_learnpath_chapter SET display_order = display_order-1 WHERE display_order > $display_order AND parent_chapter_id = $parent_id";
	$result = api_sql_query($sql, __FILE__, __LINE__);
...
function deletepath($path_id)
{
	...
	$sql = "DELETE FROM $tbl_learnpath_main WHERE learnpath_id='$path_id'";
	$result = api_sql_query($sql, __FILE__, __LINE__);

	//@TODO check how this function is used before uncommenting the following
	//also delete all elements inside that path
	$sql = "SELECT * FROM $tbl_learnpath_chapter WHERE learnpath_id=$path_id";
	$result = api_sql_query($sql, __FILE__, __LINE__);
	while ($row = mysql_fetch_array($result))
	{
		deletemodule($row['id']);
...
function moveitem($direction, $id, $moduleid, $type = 'item')
{
	...
	{
		$sql1 = "UPDATE $tbl_learnpath_chapter SET display_order = ".$dest_order." WHERE (id=$orig_id and parent_chapter_id=$moduleid)";
	}
	elseif ($orig_type == 'item')
	{
		$sql1 = "UPDATE $tbl_learnpath_item SET display_order = ".$dest_order." WHERE (id=$orig_id and chapter_id=$moduleid)";
	}
	else
	{
		return false;
	...
	{
		$sql2 = "UPDATE $tbl_learnpath_chapter SET display_order = ".$orig_order." WHERE (id='$dest_id' and parent_chapter_id=$moduleid)";
	}
	elseif ($dest_type == 'item')
	{
		$sql2 = "UPDATE $tbl_learnpath_item SET display_order = ".$orig_order." WHERE (id='$dest_id' and chapter_id=$moduleid)";
	}
	else
	{
		return false;
...
function movemodule($direction, $id)
{
	...
	// Select all chapters of first level (parent_chapter_id = 0)
	$sql = "SELECT * FROM $tbl_learnpath_chapter where (learnpath_id=$learnpath_id AND parent_chapter_id = 0) ORDER BY display_order $sortDirection";
	$result = api_sql_query($sql, __FILE__, __LINE__);
	$previousrow = "";
	...
	while ($row = mysql_fetch_array($result))
	{
		...
		if (!empty ($this_cat_order))
		{
			...
			$sql1 = "UPDATE $tbl_learnpath_chapter SET display_order = '$next_cat_order' WHERE (id='$this_cat_id' and learnpath_id=$learnpath_id)";
			$sql2 = "UPDATE $tbl_learnpath_chapter SET display_order = '$this_cat_order' WHERE (id='$next_cat_id' and learnpath_id=$learnpath_id)";
			api_sql_query($sql1, __FILE__, __LINE__);
			api_sql_query($sql2, __FILE__, __LINE__);
...
function array_learnpath_categories()
{
	...
	$sql = "SELECT * FROM  $tbl_learnpath_chapter  WHERE (learnpath_id=$learnpath_id) ORDER BY display_order ASC";
	$result = api_sql_query($sql, __FILE__, __LINE__);
...
*/
function display_learnpath_chapters($chapter_id = 0, $tree = array (), $level = 0)
{
	global $color2;
	global $xml_output;
	global $learnpath_id;
	...
	//$sql="SELECT * FROM  $tbl_learnpath_chapter  WHERE (learnpath_id=$learnpath_id AND parent_chapter_id=$chapter_id) ORDER BY display_order ASC";
	//$result=api_sql_query($sql,__FILE__,__LINE__);
	...
	$i = 1;
	$counter = 0;
	$num_modules = count($tree[$chapter_id]);

	//while ($row=mysql_fetch_array($result))
	if (isset ($tree[$chapter_id]))
	{
		foreach ($tree[$chapter_id] as $row)
		{
			if ($row['type'] === 'chapter')
			{
				$xml_output .= "<chapter>";
				$xml_output .= "<chaptertitle>".$row["chapter_name"]."</chaptertitle>";
				$xml_output .= "<chapterdescription>".$row["chapter_description"]."</chapterdescription>";
				...
				echo '<tr class="'.$oddclass.'">'."\n".'  <td>'.str_repeat("&nbsp;&gt;", $level)."<img src='../img/documents.gif' alt='folder'/><a href='".$_SERVER['PHP_SELF']."?learnpath_id=$learnpath_id&chapter_id={$row['id']}&action=add&type=learnpathitem'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9><b>&nbsp;".$row['chapter_name']."</b></a>"."<br /><i><div align='justify'>&nbsp;".str_repeat("&nbsp;&nbsp;&nbsp;", $level).$row['chapter_description']."</i></td>\n".'  <td  align="center"><a href="'.$_SERVER['PHP_SELF']."?learnpath_id=$learnpath_id&chapter_id={$row['id']}&action=add&type=learnpathitem\"&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9><img src='../img/0.gif' width='13' height='13' border='0' title='$lang_add_learnpath_item'></a></td>\n"."  <td";
				if (is_prereq($learnpath_id))
				{
					echo " bgcolor='#ddddee'";
				}
				...
				if (is_allowed_to_edit())
				{
					...
					{
						$myaction = 'moveitem&amp;type=chapter';
					}
					if ($i < $num_modules)
					{

						// if we are still under the number of chapters in this section, show "move down"
						echo "  <td align=center>"."<a href='".$_SERVER['PHP_SELF']."?learnpath_id=$learnpath_id&amp;action=".$myaction."&amp;direction=down&amp;moduleid=".$chapter_id."&amp;id=".$row["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9>"."<img src=\"../img/down.gif\" border=\"0\" title=\"$lang_move_down\">"."</a></td>\n";
					}
					else
					{
					...
					if ($i > 1)
					{
						echo '  <td align="center">'."<a href='".$_SERVER['PHP_SELF']."?learnpath_id=$learnpath_id&amp;action=".$myaction."&amp;direction=up&amp;moduleid=".$chapter_id."&amp;id=".$row["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9>"."<img src=\"../img/up.gif\" border=\"0\" title=\"$lang_move_up\">"."</a>"."</td>\n";
					}
					else
					{
						echo '  <td align="center">&nbsp;</td>'."\n";
					...
					echo "  <td align='center'>&nbsp;</td>\n";
					echo "  <td align='center'>"."<a href='".$_SERVER['PHP_SELF']."?learnpath_id=$learnpath_id&amp;action=editmodule&amp;id=".$row["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9>"."<img src=\"../img/edit.gif\" border=\"0\" title=\"$lang_edit_learnpath_module\">"."</a>"."</td>\n";
					echo "  <td align='center'>"."<a href='".$_SERVER['PHP_SELF']."?learnpath_id=$learnpath_id&amp;action=deletemodule&amp;id=".$row["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9 onclick=\"return confirmation('".$row['chapter_name']."');\">"."<img src=\"../img/delete.gif\" border=\"0\" title=\"$lang_delete_learnpath_module\">"."</a>"."</td>\n";
				}
			...
			elseif ($row['type'] === 'item')
			{
				...
				display_addedresource_link_in_learnpath($row_items["item_type"], $row_items["item_id"], '', $row_items["id"], 'builder', 'icon', $level);
				...
				if (is_allowed_to_edit())
				{
					...
					if ($row_items["prereq_id"] <> '')
					{
						$prereq = $row_items["prereq_id"];

						if ($row_items["prereq_type"] == 'i')
						{
							//item
							$sql_items2 = "SELECT * FROM $tbl_learnpath_item WHERE id='$prereq'"; //check if prereq has been deleted
							$result_items2 = api_sql_query($sql_items2, __FILE__, __LINE__);
							$number_items2 = mysql_num_rows($result_items2);
							if ($number_items2 == 0)
							{
								echo "<font color='red'>$lang_prereq_deleted_error</font>";
							}
							$row_items2 = mysql_fetch_array($result_items2);
							display_addedresource_link_in_learnpath($row_items2["item_type"], $row_items2["item_id"], '', $row_items2["id"], 'builder', '', $level);
							if ((($row_items2["item_type"] == 'Exercise') or ($row_items2["item_type"] == 'HotPotatoes')) and ($row_items['prereq_completion_limit']))
							{
								echo "&nbsp;({$row_items['prereq_completion_limit']})";
							}
						}
					...
					if ($i < $num_modules)
					{
						echo "<td align='center'>"."<a href='".$_SERVER['PHP_SELF']."?learnpath_id=$learnpath_id&amp;action=moveitem&amp;type=item&amp;direction=down&amp;moduleid=".$chapter_id."&amp;id=".$row_items["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9>"."<img src=\"../img/down.gif\" border=\"0\" title=\"$lang_move_down\">"."</a>"."</td>";
					}
					else
					{
					...
					if ($i > 1)
					{
						echo "<td align='center'>"."<a href='".$_SERVER['PHP_SELF']."?learnpath_id=$learnpath_id&amp;action=moveitem&amp;type=item&amp;direction=up&amp;moduleid=".$chapter_id."&amp;id=".$row_items["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9>"."<img src=\"../img/up.gif\" border=\"0\" title=\"$lang_move_up\">"."</a>";
					}
					else
					{
						echo "<td width='30' align='center'>&nbsp;</td>";
					...
					// edit prereq
					echo "<a href='".$_SERVER['PHP_SELF']."?learnpath_id=$learnpath_id&amp;action=edititemprereq&amp;id=".$row_items["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9>"."<img src=\"../img/scormpre.gif\" border=\"0\" title=\"$lang_add_prereq\">"."</a>"."</td>";

					// edit
					echo "<td align='center'>"."<a href='".$_SERVER['PHP_SELF']."?learnpath_id=$learnpath_id&amp;action=edititem&amp;id=".$row_items["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9>"."<img src=\"../img/edit.gif\" border=\"0\" title=\"$lang_edit_learnpath_item\">"."</a>"."</td>";

					// delete
					echo "<td align='center'>"."<a href='".$_SERVER['PHP_SELF']."?learnpath_id=$learnpath_id&action=deleteitem&id=".$row_items["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9>"."<img src=\"../img/delete.gif\" border=\"0\" title=\"$lang_delete_learnpath_item\" onclick=\"return confirmation('".$langThisItem."');\">"."</a>";
				}
				$i ++;
				echo "</td></tr>";
...
function display_all_learnpath()
{
	...
	while ($row = mysql_fetch_array($result))
	{
		...
		echo "<a href='learnpath_handler.php?&learnpath_id={$row['learnpath_id']}'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9>{$row['learnpath_name']}</a>";
		echo "</b><br><i><div class=text align=justify>{$row['learnpath_description']}</div></i></td>";

		// showing the edit, delete and publish icons
		if (is_allowed_to_edit())
		{
			echo "<td bgcolor=\"$color2\" align=center><a href='learnpath_handler.php?learnpath_id={$row['learnpath_id']}&action=add&type=learnpathcategory'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9><img src='../img/0.gif' width='13' height='13' border='0' title='$lang_add_learnpath_chapter_to_path'></a></td>";
			echo "<td bgcolor=\"$color2\" align=center><a href='".$_SERVER['PHP_SELF']."?action=editpath&id=".$row["learnpath_id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9><img src=\"../img/edit.gif\" border=\"0\" title=\"$lang_edit_learnpath\"></a></td>";
			echo "<td bgcolor=\"$color2\" align=center><a href='".$_SERVER['PHP_SELF']."?action=deletepath&id=".$row["learnpath_id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9><img src=\"../img/delete.gif\" border=\"0\" title=\"$lang_delete_learnpath\" onclick=\"return confirmation('".$row2['learnpath_name']."');\"></a></td>";
			$id = $row["learnpath_id"];
			...
			{
				echo "<td bgcolor=\"$color2\" align=center><a href='".$_SERVER['PHP_SELF']."?action=publishpath&set=i&id=".$row["learnpath_id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9><img src=\"../img/visible.gif\" border=\"0\" title=\"$lang_no_publish\"></a></td>";
			}
			else
			{
				echo "<td bgcolor=\"$color2\" align=center><a href='".$_SERVER['PHP_SELF']."?action=publishpath&set=v&id=".$row["learnpath_id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9><img src=\"../img/invisible.gif\" border=\"0\" title=\"$lang_publish\"></a></td>";
			}
		}
		$i ++;
		echo "<tr><td>&nbsp;</td></tr>";
...
function display_learnpath_items($categoryid)
{
	...
	$sql_items = "SELECT * FROM $tbl_learnpath_item WHERE chapter_id='$categoryid' ORDER BY display_order ASC";
	$result_items = api_sql_query($sql_items);
	$number_items = mysql_num_rows($result_items);
	$i = 1;

	while ($row_items = mysql_fetch_array($result_items))
	{
		echo "<tr><td colspan='2' valign='top'>";
		display_addedresource_link_in_learnpath($row_items["item_type"], $row_items["item_id"], '', $row_items["id"], 'builder', 'icon');
		if ($row_items["description"])
		{
			echo "<div align='justify'><font color='#999999'>&nbsp;&nbsp;&nbsp;{$row_items['description']}</font>";
		}
		...
		if (is_allowed_to_edit())
		{
			...
			if ($row_items["prereq_id"] <> '')
			{
				...
				if ($row_items["prereq_type"] == 'i')
				{
					...
					$row_items2 = mysql_fetch_array($result_items2);
					display_addedresource_link_in_learnpath($row_items2["item_type"], $row_items2["item_id"], '', $row_items2["id"], 'builder', '');
					if ((($row_items2["item_type"] == 'Exercise') or ($row_items2["item_type"] == 'HotPotatoes')) and ($row_items['prereq_completion_limit']))
					{
						echo "&nbsp;({$row_items['prereq_completion_limit']})";
					}
			...
			if ($i < $number_items)
			{
				echo "<td align='center'><a href='".$_SERVER['PHP_SELF']."?learnpath_id=$learnpath_id&action=moveitem&direction=down&moduleid=".$categoryid."&id=".$row_items["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9><img src=\"../img/down.gif\" border=\"0\" title=\"$lang_move_down\"></a></td>";
			}
			else
			{
				echo "<td width='30' align='center'>&nbsp;</td>";
			...
			if ($i > 1)
			{
				echo "<td align='center'><a href='".$_SERVER['PHP_SELF']."?learnpath_id=$learnpath_id&action=moveitem&direction=up&moduleid=".$categoryid."&id=".$row_items["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9><img src=\"../img/up.gif\" border=\"0\" title=\"$lang_move_up\"></a>";
			}
			else
			{
				echo "<td width='30' align='center'>&nbsp;</td>";
			...
			echo "<a href='".$_SERVER['PHP_SELF']."?learnpath_id=$learnpath_id&action=deleteitem&id=".$row_items["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9><img src=\"../img/delete.gif\" border=\"0\" title=\"$lang_delete_learnpath_item\" onclick=\"return confirmation('".$langThisItem."');\"></a>";
		}
		$i ++;
		echo "</td></tr>";
	}
...
function learnpath_items($itemid)
{
	...
	$sql_items = "SELECT chapter_id FROM $tbl_learnpath_item WHERE id='$itemid'";
	$moduleid_sql = api_sql_query($sql_items);
	$moduleid_array = mysql_fetch_array($moduleid_sql); //first row of the results
	$moduleid = $moduleid_array["chapter_id"];

	$sql_items = "SELECT * FROM $tbl_learnpath_item WHERE chapter_id='$moduleid' ORDER BY display_order ASC";
	$result_items = api_sql_query($sql_items);
...
function learnpath_chapters($learnpath_id)
{
	...
	$sql_items = "SELECT * FROM $tbl_learnpath_chapter WHERE learnpath_id='$learnpath_id' ORDER BY display_order ASC";
...
/**
 * This function tells if a learnpath contains chapters which are prerequisite to other items
 * @param	integer	Learnpath id
 * @return	boolean	True if this learnpath contains a chapter which is a prerequisite to something
 */
function is_prereq($learnpath_id)
{
	...
	{
		$c = $ar['id'];
		$sql_items2 = "SELECT * FROM $tbl_learnpath_item WHERE chapter_id='$c' ORDER BY display_order ASC";
		$result_items2 = api_sql_query($sql_items2);
		$ar2 = mysql_fetch_array($result_items2);
		while ($ar2 != '')
		{
			if ($ar2['prereq_id'] != '')
			{
...
function prereqcheck($id_in_path)
{
	...
	if ($row['prereq_type'] == 'i')
	{
		...
		$stepname = display_addedresource_link_in_learnpath($row['item_type'], $row['item_id'], '', $id_in_path, 'builder', 'nolink');
		...
		$prereqname = display_addedresource_link_in_learnpath($row6['item_type'], $row6['item_id'], '', $id_in_path3, 'builder', 'nolink'); //this is the prereq of the step we want to open
	...
	elseif ($row['prereq_type'] == 'c')
	{
		...
		$sql_items3 = "SELECT * FROM $tbl_learnpath_item WHERE chapter_id='$id_in_path2'";
		$result_items3 = api_sql_query($sql_items3);
		$allcompleted = true;
		...
		else
		{
			...
			$sql5 = "SELECT * FROM $tbl_learnpath_chapter WHERE (learnpath_id='$learnpath_id' and id='$id_in_path2')";
...
function get_learnpath_tree($learnpath_id)
{
	...
	$sql = "SELECT * FROM $tbl_learnpath_chapter WHERE learnpath_id = ".$learnpath_id." ORDER BY id";
	...
	while ($row = mysql_fetch_array($res, MYSQL_ASSOC))
	{
		...
		if (empty ($row['parent_chapter_id']))
		{
			$row['parent_chapter_id'] = 0;
		}
		//$chapters_by_parent[$row['parent_chapter_id']][$row['display_order']] = $row;
		$all_items_by_chapter[$row['parent_chapter_id']][$row['display_order']] = $row;
		$all_items_by_chapter[$row['parent_chapter_id']][$row['display_order']]['type'] = 'chapter';
	}
	...
	foreach ($chapters as $row)
	{
		...
		$sql = "SELECT * FROM $tbl_learnpath_item WHERE chapter_id = ".$row['id']." ORDER BY id";
		$res = api_sql_query($sql, __FILE__, __LINE__);
		while ($myrow = mysql_fetch_array($res, MYSQL_ASSOC))
		{
			//$items[] = $myrow;
			//$items_by_chapter[$myrow['chapter_id']][$myrow['display_order']] = $myrow;
...
function get_ordered_items_list($tree, $chapter = 0, $include_chapters = false)
{
	...
	foreach ($tree[$chapter] as $order => $elem)
	{
		...
		{
			$list[] = array ('id' => $elem['id'], 'type' => $elem['type'], 'item_type' => $elem['item_type'], 'chapter_id' => $elem['chapter_id'], 'item_id' => $elem['item_id']);
		}
	}
	return $list;
}
...
function display_toc_chapter_contents($tree, $chapter_id = 0, $learnpath_id, $uid, $wrap, $level = 0)
{
	...
	$num = 0;
	$num_completed = 0;
	foreach ($tree[$chapter_id] as $order => $elem)
	{
		...
		elseif ($elem['type'] === 'item')
		{
			...
			$sql0 = "SELECT * FROM $tbl_learnpath_user WHERE (user_id='".$uid."' and learnpath_item_id='".$elem['id']."' and learnpath_id='".$learnpath_id."')";
			...
			display_addedresource_link_in_learnpath($elem['item_type'], $elem['item_id'], $completed, $elem['id'], 'player', $icon);
			if ($bold)
			{
				echo "</b>";
			}
 ...
 **/
function get_tracking_table($learnpath_id, $user_id, $chapter_id = 0, $tree = false, $level = 0, $counter = 0)
{
	$tbl_learnpath_chapter = Database :: get_course_learnpath_chapter_table();
	...
	foreach ($tree[$chapter_id] as $order => $elem)
	{
		...
		elseif ($elem['type'] == 'item')
		{
			...
			$sql = "SELECT * FROM $tbl_learnpath_user "."WHERE user_id = $user_id "."AND learnpath_id = $learnpath_id "."AND learnpath_item_id = ".$elem['id'];
			$res = api_sql_query($sql, __FILE__, __LINE__);
			...
			display_addedresource_link_in_learnpath($elem['item_type'], $elem['item_id'], $myrow['status'], $elem['id'], 'player', 'wrap');
...
function is_empty($id)
{
	...
	$sql = "SELECT * FROM $tbl_learnpath_chapter WHERE learnpath_id=$id ORDER BY display_order ASC";
	$result = api_sql_query($sql, __FILE__, __LINE__);
	$num_modules = mysql_num_rows($result);
	$empty = true;
	...
	if ($num_modules != 0)
	{
		while ($row = mysql_fetch_array($result))
		{
			...
			$num_items = 0;
			$chapter_id = $row['id'];
			$sql2 = "SELECT * FROM $tbl_learnpath_item WHERE (chapter_id=$chapter_id) ORDER BY display_order ASC";
			$result2 = api_sql_query($sql2, __FILE__, __LINE__);
	...
	*/
	...
	while ($circle1_files[0][$i])
	{
		...
		if ($chapter_id != $previous_chapter_id)
		{
			...
			$sql2 = "SELECT * FROM $tbl_learnpath_chapter WHERE (id=".$chapter_id.")";
			$result2 = api_sql_query($sql2, __FILE__, __LINE__);
			$row2 = mysql_fetch_array($result2);
