    ...
    if ($upload_ok) {
        ...
        if ($new_path && ($new_comment || $new_title)) {
            if (($docid = DocumentManager::get_document_id($_course, $new_path))) {
                ...
                Database::query(
                    "UPDATE $table_document SET".substr($ct, 1).
                        " WHERE id = '$docid'"
                );
            }
        }
        //check for missing images in html files
        $missing_files = FileManager::check_for_missing_files($base_work_dir.$_POST['curdirpath'].$new_path);
        if ($missing_files) {
            //show a form to upload the missing files
            Display::display_normal_message(
                FileManager::build_missing_files_form(
                    $missing_files,
                    $_POST['curdirpath'],
...
if (isset($_POST['submit_image'])) {
    ...
    if ($number_of_uploaded_images > 0) {
        ...
        $missing_files_dir = FileManager::create_unexisting_directory(
            $_course,
            $_user['user_id'],
            api_get_session_id(),
            $to_group_id,
        ...
        $paths_to_replace_in_file = FileManager::move_uploaded_file_collection_into_directory(
            $_course,
            $_FILES['img_file'],
            $base_work_dir,
            $missing_files_dir,
        ...
        FileManager::replace_img_path_in_html_file(
            $_POST['img_file_path'],
...
<form action="<?php echo api_get_self(); ?>" method="POST" name="upload" enctype="multipart/form-data">
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                - <input type="checkbox" name="unzip" value="1" onclick="check_unzip()"/> <?php echo(get_lang(
                'Uncompress'
            ));?><br/>
                - <?php echo (get_lang('UplWhatIfFileExists'));?><br/>
                &nbsp;&nbsp;&nbsp;<input type="radio" name="if_exists" value="nothing"
                                         title="<?php echo (get_lang('UplDoNothingLong'));?>"
                                         checked="checked"/>  <?php echo (get_lang('UplDoNothing'));?><br/>
                &nbsp;&nbsp;&nbsp;<input type="radio" name="if_exists" value="overwrite"
                                         title="<?php echo (get_lang('UplOverwriteLong'));?>"/> <?php echo (get_lang(
                'UplOverwrite'
            ));?><br/>
                &nbsp;&nbsp;&nbsp;<input type="radio" name="if_exists" value="rename"
                                         title="<?php echo (get_lang('UplRenameLong'));?>"/> <?php echo (get_lang(
                'UplRename'
            ));?>
...
<p><?php echo (get_lang('Back'));?> <?php echo (get_lang('To'));?> <a
