...
abstract class CcHelpers
{
    ...
    public static function addFiles(CcIManifest &$manifest, $packageroot, $outdir, $allinone = true, $folder = null, $docfilepath = null): void
    {
        $permDirs = api_get_permissions_for_new_directories();

        $files = self::embeddedMapping($packageroot, null, $folder, $docfilepath);
        $basePath = api_get_path(SYS_APP_PATH);
        ...
        foreach ($files as $virtual => $values) {
            ...
            PkgStaticResources::instance()->add(
                $virtual,
                $res[0],
        ...
        if (!empty($lfiles)) {
            foreach ($lfiles as $lfile) {
                ...
                if (isset($files[$bfile])) {
                    ...
                    $content = str_replace(
                        $internalCourseDocumentsPath.$filename,
