<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
      ...
      <Line />
      <Step :index="3" :current="step" :label="t('Create .mbz')" />
    </div>
    ...
    <section v-if="step===1" class="grid grid-cols-1 gap-4 lg:grid-cols-2">
      <div class="rounded-lg border border-gray-25 p-4">
        ...
        <CMAlert type="warning" class="mt-3"
      ...
      <div class="rounded-lg border border-gray-25 p-4 space-y-3">
        ...
        <select v-model="moodleVersion" class="w-full rounded border border-gray-25 p-2 text-sm">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div class="sm:col-span-1">
            ...
            <input v-model.trim="adminId" class="w-full rounded border border-gray-25 p-2 text-sm"
          ...
          <div class="sm:col-span-1">
            ...
            <input v-model.trim="adminLogin" class="w-full rounded border border-gray-25 p-2 text-sm"
          ...
          <div class="sm:col-span-2">
            ...
            <input v-model.trim="adminEmail" class="w-full rounded border border-gray-25 p-2 text-sm"
                   type="email" :placeholder="t('Email to use in the archive metadata')" />
          </div>
        </div>
      ...
      <div class="col-span-full flex justify-end gap-3">
        ...
        <button class="btn-primary" :disabled="loading || !canContinueFromStep1" @click="nextFromStep1">
...
<script setup>
/* Strings in English as requested */
...
onMounted(async () => {
  try {
    ...
    // soft fail – options are optional
  } finally {
    loading.value = false
  }
})
...
async function doExport() {
  ...
  try {
    ...
    error.value = e?.response?.data?.error || t("Failed to create Moodle backup.")
  } finally {
    loading.value = false
  }
}
