...
$PermissionsForNewFilesComment = "Die Möglichkeit, Berechtigungen für alle neu erstellten Dateien zu definieren, erhöht die Sicherheit gegen Hacker-Angriffe, falls gefährlicher Inhalt auf ihr Portal hochgeladen wird. Die vordefinierten Einstellungen (0550) sollten genügen, um ihrem Server";
...
$SplitUsersUploadDirectoryComment = "Bei hochfrequentierten Portalen kann das Upload-Verzeichnis (main/upload/users) zu viele Dateien für das Dateisystem enthalten (es gab schon mehr als 36.000 Dateien auf einem Debian Server). Das Ändern dieser Option ermöglicht die Aufteilung von Verzeichn";
...
$DropboxAllowStudentToStudentComment = "Teilnehmern gestatten, Dokumente an andere Teilnehmer zu verschicken (Peer 2 Peer, P2P Austausch). Teilnehmer können dies für weniger relevante Dokumente benutzen (mp3, Lösungen ...). Wenn dies deaktiviert wird, können Teilnehmer Dokumente nur an Kursleit";
...
$PermissionsForNewDirsComment = "Die Möglichkeit, Berechtigungen für alle neu erstellten Verzeichnisse zu definieren, erhöht die Sicherheit gegen Hacker-Angriffe, falls gefährlicher Inhalt auf Ihr Portal hochgeladen wird. Die vordefinierten Einstellungen (0770) sollten genügen, um Ihrem";
...
$langVisioconfDescription = "Chamilo Live Conferencing® ist ein Standard Tool von Visioconference. Es bietet: Anzeige von Diapos, Whiteboard zum Zeichnen und Schreiben, Audio/Video duplex, Chat. Es ist lediglich der Flash® Player notwendig und es werden 3 Modi unterstützt: one2one, o";
...
$VersionCheckExplanation = "Um die automatische Versionsbenachrichtigung zu nutzen, muss man seine Chamilo-Installation bei chamilo.com registrieren. Die weitergegebene Information nach Klicken dieses Buttons dient lediglich internen Zwecken und lediglich Gesamtinformationen werden";
...
$PublicPagesComplyToWAIComment = "WAI (Web Accessibility Initiative) ist eine Initiative, um Webseiten besser zugänglich zu machen. Durch Auswahl dieser Option werden die öffentlichen Chamilo-Seiten besser zugänglich. Das bedeutet auch, dass einige Inhalte auf den öffentlichen Seiten des";
...
$YesAddCSVHeader = "Ja, den CSV Header hinzufügen<br/>Diese Zeile definiert Felder und ist notwendig, wenn man die Datei in ein anderes Chamilo Campus importieren will.";
...
$UploadExtensionsBlacklistComment = "Die Blacklist wird benutzt, um Dateierweiterungen zu filtern, indem Dateien gelöscht (bzw. umbenannt) werden, deren Erweiterung in der Blacklist enthalten ist. Die Erweiterungen sollten ohne den Punkt (.) angegeben und durch ein Semikolon (;) getrennt wer";
$UploadExtensionsWhitelistComment = "Die Whitelist wird zum Filtern von Dateien benutzt zum Löschen (bzw. umbenennen) der Dateien, die *NICHT* in der Whitelist angegeben sind. Dies wird allgemein als eine sicherere, aber striktere Filtermethode angesehen. Die Erweiterungen sollten ohne den P";
$UploadExtensionsSkip = "Filterverhalten (überspringen/umbenennen)";
$UploadExtensionsSkipComment = "Wenn überspringen gewählt wird, werden die durch die Blacklist oder Whitelist gefilterten Dateien nicht zur Plattform hochgeladen. Wenn umbenennen gewählt wird, wird die Erweiterung ersetzt durch die in der Ersetzungseinstellung angegebene Dateierweiterun";
...
$EphorusDescription = "Beginnen Sie, den Ephorus Anti-Plagiat Dienst in Chamilo zu benutzen. <br/><strong>Mit Ephorus können Sie Internet Plagiate ohne zusätzliche Bemühungen verhindern.</strong><br/> Sie können unseren einzigartigen Open Standard Webservice benutzen, um ihre e";
...
$PlatformCharsetComment = "Der Zeichensatz bestimmt die Art und Weise, in der bestimmte Sprachen in Chamilo dargestellt werden. Wenn man z.B. kyrillische oder japanische Zeichen benutzt, sollte man vielleicht den Zeichensatz ändern. Für englische, lateinische und westeuropäische Sc";
...
$NoReplyEmailAddressComment = "Dies ist die E-Mail Adresse, die benutzt wird, wenn eine E-Mail verschickt wird, auf die gemäß besonderem Wunsch keine Antwort geschickt werden soll.  Gewöhnlich sollte diese E-Mail Adresse auf dem Server so konfiguriert sein, dass sie jede eingehende Mai";
...
$InstallDirAccessibleSecurityThreat = "Das Installations-Verzeichnis Ihrer Installation (./main/install) ist noch immer für Web-Benutzer erreichbar. Das könnte ein Sicherheitsrisiko für Ihre Installation darstellen. Wir raten Ihnen, dieses Verzeichnis zu löschen oder die Berechtigungen so zu ä";
...
$UserThemeSelectionComment = "Benutzer können ihr eigenes Theme in ihrem Profil auswählen. Das verändert das Aussehen von Chamilo für sie, lässt aber den Stil des Portals intakt. Falls ein bestimmter Kurs oder eine Session ein bestimmtes Theme verwendet, hat dies Vorrang über Benutzer";
$AllowurlfopenIsSetToOff = "Die PHP Auswahl \\"allow_url_fopen\\" steht auf off. Dies verhindert die korrekte Funktion der Anmeldung. Diese Auswahl kann in der PHP Konfigurationsdatei (php.ini) oder in der Apache Virtual Host Konfiguration mittels des Befehls php_admin_value geändert";
...
$UserFieldsAddHelp = "Ein Benutzerfeld hinzufügen ist sehr einfach:<br/>-einzelnes Wort in Kleinbuchstaben als eindeutigen Namen auswählen,<br/>- Typus auswählen,<br/>-einen Text wählen, der dem Benutzer angezeigt wird (nimmt man einen existierenden übersetzten Begriff wie Bir";
$AllowCourseThemeTitle = "Themen für Kurse erlauben";
$AllowCourseThemeComment = "Grafische Themen für Kurse zu erlauben, ermöglicht das Verändern des von einem Kurs benutzten Stylesheets zu einem anderen in Chamilo verfügbaren Stylesheet. Wenn ein Benutzer den Kurs aufruft, hat das Stylesheet des Kurses Priorität über das vom Benutzer";
...
$ShowClosedCoursesComment = "Abgeschlossene Kurse auf der Loginseite bzw Startseite anzeigen? Auf der Campus Startseite wird ein Icon bei dem Kurs eingeblendet, wo der Kurs kurz beschrieben wird. Dies wird nur auf der Campus Startseite angezeigt, wenn ein Benutzer eingeloggt aber noc";
$LDAPConnectionError = "LDAP Verbindungsfehler";
$LDAP = "LDAP";
$LDAPEnableTitle = "LDAP aktivieren";
$LDAPEnableComment = "Bei Nutzung eines LDAP Servers müssen die folgenden Einstellungen konfiguriert und die Konfigurationsdatei entsprechend der Anweisungen im Installationshandbuch eingerichtet werden, danach diese aktivieren. Dann können Benutzer sich mit ihrem LDAP Benutze";
...
$LDAPFilledTutorFieldComment = "Für dieses LDAP Kontatkfeld wird ein Check ausgeführt bei der Eintragung neuer Benutzer. Ist das Feld nicht leer, wird der Benuter als Tutor betrachtet und als solcher in Chamilo eingefügt. Will man alle Benutzer als einfache Benuter erkennen lassen, das";
...
$VisioUseRtmptComment = "Das rtmpt Protokoll ermöglicht Videokonferenz von hinter einer Firewall, indem die Kommunikation über Port 80 weitergeleitet wird. Dadurch wird allerdings das Streaming langsamer, weshalb es empfehlenswert ist, dies nur dann zu benutzen, wenn es notwendig";
...
$LDAPFilledTutorFieldValueComment = "Gibt es einen Check für das obige Tutor Feld, muss der Wert innerhalb eines der Unterlemente des Tutor Feldes sein, damit der Benutzer als ein Lehrer eingeordnet wird. Wird das Feld leer gelassen, dann ist die einzige Bedingung, dass das Feld für diesen L";
