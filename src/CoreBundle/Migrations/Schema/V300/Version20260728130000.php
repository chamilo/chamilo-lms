...
/**
 * First fixtures-upsert migration for the 3.0 (V300) series: re-run of
 * Version20260721125301's (V210) fixtures-upsert logic, so that any setting
 * added to SettingsCurrentFixtures.php since then (here: the OAuth
 * authorization server opt-in toggle, "security.oauth_server_enabled") gets
 * its settings/settings_current row created on existing installs — a fresh
 * install already gets every fixture-defined setting for free via
 * SettingsManager::installSchemas(), but an upgrade only runs migrations, not
 * fixtures.
 *
 * Deliberately kept as its own migration, separate from the one that adds the
 * new oauth_client/oauth_authorization_code/oauth_access_token/
 * oauth_refresh_token tables — this fixtures-upsert logic is a recurring
 * pattern that will be copied forward again for future settings (and may
 * itself be renamed), so it should never be bundled with unrelated schema DDL.
 */
final class Version20260728130000 extends AbstractMigrationChamilo
{
    public function getDescription(): string
    {
        return 'Upsert settings (title/comment/category/value) + settings_value_template from current fixtures.';
    }
