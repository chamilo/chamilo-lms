...
function delete_category($action, $id, $user_id = null)
{
    ...
    $sql = "DELETE FROM ".$dropbox_cnf['tbl_category']." WHERE c_id = $course_id AND cat_id='".intval(
        $id
    )."' AND $sentreceived='1'";
...
{
    $form = new FormValidator('form1', 'post', api_get_self().'?view_received_category='.Security::remove_XSS(
        $_GET['view_received_category']
    ).'&view_sent_category='.Security::remove_XSS($_GET['view_sent_category']).'&view='.Security::remove_XSS(
        $_GET['view']
    ).'&'.$extra_params);
...
function display_file_checkbox($id, $part)
{
    ...
    $return_value = '<input type="checkbox" name="'.Security::remove_XSS($part).'_'.Security::remove_XSS(
        $id
    ).'" value="'.Security::remove_XSS($id).'" '.$checked.' />';
...
function store_addcategory()
{
    ...
    if (!$_POST['edit_id']) {
        ...
        $sql = "SELECT * FROM ".$dropbox_cnf['tbl_category']." WHERE c_id = $course_id AND user_id='".$_user['user_id']."' AND cat_name='".Database::escape_string(
            $_POST['category_name']
        )."' AND received='".$received."' AND sent='$sent' AND session_id='$session_id'";
        $result = Database::query($sql);
        ...
        if (Database::num_rows($result) == 0) {
            $sql = "INSERT INTO ".$dropbox_cnf['tbl_category']." (c_id, cat_name, received, sent, user_id, session_id)
                ...
                $_POST['category_name']
            )."', '".Database::escape_string($received)."', '".Database::escape_string(
                $sent
            )."', '".Database::escape_string($_user['user_id'])."',$session_id)";
            Database::query($sql);
        ...
        $sql = "UPDATE ".$dropbox_cnf['tbl_category']." SET cat_name='".Database::escape_string(
            $_POST['category_name']
        )."', received='".Database::escape_string($received)."' , sent='".Database::escape_string($sent)."'
...
function display_addcategory_form($category_name = '', $id = '', $action)
{
    ...
    if (isset($id) AND $id != '') {
        ...
        $sql = "SELECT * FROM ".$dropbox_cnf['tbl_category']." WHERE c_id = $course_id AND cat_id='".Database::escape_string(
            $id
        )."'";
        $result = Database::query($sql);
...
function display_add_form()
{
	...
	<form method="post" action="index.php?view_received_category=<?php echo Security::remove_XSS(
        $_GET['view_received_category']
    ); ?>&view_sent_category=<?php echo Security::remove_XSS(
        $_GET['view_sent_category']
    ); ?>&view=<?php echo Security::remove_XSS($_GET['view']); ?>&<?php echo "origin=$origin"."&".api_get_cidreq(); ?>"
          enctype="multipart/form-data" onsubmit="javascript: return checkForm(this);">
        ...
        <div class="control-group">
            ...
            <div class="controls">
                ...
                <input type="file" name="file" size="20" <?php if (dropbox_cnf(
                    'allowOverwrite'
                )
                ) {
                    echo 'onChange="javascript: checkfile(this.value);"';
                } ?> />
	...
	*/
	...
	foreach ($complete_user_list_for_dropbox as $current_user) {
        ...
        ) && $current_user['user_id'] != $_user['user_id']
        ...
        if (count($complete_group_list_for_dropbox) > 0) {
            foreach ($complete_group_list_for_dropbox as $current_group) {
                if ($current_group['number_of_members'] > 0) {
                    echo '<option value="group_'.$current_group['id'].'">G: '.$current_group['name'].' - '.$current_group['number_of_members'].' '.get_lang(
                        'Users'
                    ).'</option>';
                }
            }
...
function getUserNameFromId($id)
{
    ...
    ) ? "CONCAT(firstname,' ', lastname)" : "CONCAT(lastname,' ', firstname)")." AS name
...
function store_add_dropbox()
{
    ...
    if ($b_send_mail) {
        foreach ($new_work_recipients as $recipient_id) {
            ...
            api_mail_html(
                ...
                get_lang('NewDropboxFileUploadedContent').' '.api_get_path(
                    WEB_CODE_PATH
    ...
    new Dropbox_SentWork($_user['user_id'], $dropbox_title, $_POST['description'], strip_tags(
        $_POST['authors']
    ), $dropbox_filename, $dropbox_filesize, $new_work_recipients);
...
function display_user_link_work($user_id, $name = '')
{
    if ($user_id != 0) {
        if (empty($name)) {
            ...
            return '<a href="../user/userInfo.php?uInfo='.$row['user_id'].'">'.api_get_person_name(
                $row['firstname'],
                $row['lastname']
            ).'</a>';
        } else {
...
function feedback_form()
{
    ...
    if ($number_users_who_see_file) {
        ...
        $return .= '<br /><button type="submit" class="add" name="store_feedback" value="'.get_lang('Ok').'"
					onclick="javascript: document.form_dropbox.attributes.action.value = document.location;">'.get_lang(
            'AddComment'
        ).'</button>';
    } else {
...
function store_feedback()
{
    ...
    if (empty($_POST['feedback'])) {
        ...
        $sql = "INSERT INTO ".$dropbox_cnf['tbl_feedback']." (c_id, file_id, author_user_id, feedback, feedback_date) VALUES
			  ($course_id, '".intval($_GET['id'])."','".api_get_user_id()."','".Database::escape_string(
            $_POST['feedback']
...
function generate_html_overview($files, $dont_show_columns = array(), $make_link = array())
{
    ...
    $return .= '<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="'.api_get_language_isocode(
