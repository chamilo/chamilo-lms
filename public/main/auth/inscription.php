...
if ('true' === api_get_setting('allow_terms_conditions')) {
    ...
    if (!api_is_anonymous()) {
        ...
        if (1 == $term_preview['type']) {
            ...
            $form->addRule('legal_accept',
                get_lang('We need you to accept our treatment of your data in order to provide you with this service. If you want to register an account, please accept the terms and click Submit. If you don\'t accept, no personal data will be treated by us about you.'),
                'required');
        } else {
...
// Blocks page because there's any action to do.
    ...
    if ($form->validate()) {
        ...
        if (CustomPages::enabled() && CustomPages::exists(CustomPages::REGISTRATION)) {
            ...
            $tpl = new Template($tool_name);
            $pageHeader = (false !== $termActivated) ? Display::page_header($tool_name) : '';
            $tpl->assign('inscription_header', $pageHeader);
            $tpl->assign('inscription_content', $content);
    ...
    $tpl->assign('form', $form->returnForm());
    $tpl->assign('hide_header', $hideHeaders);
    $tpl->assign('text_after_registration', '');
