<?php

declare(strict_types=1);
$strings['plugin_title'] = 'Sell courses';
$strings['plugin_comment'] = 'Sell courses directly through your Chamilo portal, using a PayPal account to receive funds. Neither the Chamilo association nor the developers involved could be considered responsible of any issue you might suffer using this plugin.';
$strings['show_main_menu_tab'] = 'Show tab in main menu';
$strings['show_main_menu_tab_help'] = 'In case of not wanting to show the tab, you can create this link in your Chamilo homepage : %s';
$strings['public_main_menu_tab'] = 'Show tab in main menu to anonymous users too';
$strings['include_sessions'] = 'Include sessions';
$strings['paypal_enable'] = 'Enable PayPal';
$strings['commissions_enable'] = 'Enable Commissions';
$strings['transfer_enable'] = 'Enable bank transfer';
$strings['unregistered_users_enable'] = 'Allow anonymous users';
$strings['invoicing_enable'] = 'Enable Invoicing';
$strings['tax_enable'] = 'Enable Taxation';
$strings['Free'] = 'FREE';
$strings['PaypalPayoutCommissions'] = 'Paypal Payout Commissions';
$strings['MyPayouts'] = 'My payments';
$strings['Commission'] = 'Commission';
$strings['Commissions'] = 'Commissions';
$strings['SetCommissions'] = 'Set commissions';
$strings['CommissionsConfig'] = 'Configure Commissions';
$strings['PayoutReport'] = 'Payouts report';
$strings['Stats'] = 'Statistics';
$strings['InfoCommissions'] = 'Enter here the sales commission as a percentage (% ) , the organization that controls the platform. This percentage is deducted from the amount received by teachers for each course or Session sold on the platform.';
$strings['NeedToSelectPaymentType'] = 'Need to select payment type';
$strings['IndividualPayout'] = 'Individual payout';
$strings['CancelPayout'] = 'Cancel payout';
$strings['ContinuePayout'] = 'Continue Payout';
$strings['ProceedPayout'] = 'Proceed with payment';
$strings['TheActualPlatformCommissionIsX'] = 'The current platform commission is <b> %s </b>. This percentage will be discounted from the total product price. The difference will be the amount to apply the following commissions.';
$strings['CoursesInSessionsDoesntDisplayHere'] = "The courses which are inside a training session don't appear in the list of courses to configure as individual products.";
$strings['WantToSellCourses'] = 'Do you want to teach and earn some money with your own courses? This list could be filled with your earnings selling courses through this platform. Get in touch with us.';
$strings['SelectOptionToProceed'] = 'Select option to proceed';
$strings['VerifyTotalAmountToProceedPayout'] = "Please check the total amount to proceed with the payment of commissions. This amount doesn't take extra costs for PayPal into account. The courses sales that do not count with a Paypal account for the beneficiaries will not be considered.";
$strings['TotalAcounts'] = 'Total of accounts:';
$strings['TotalPayout'] = 'Total to be paid:';
$strings['PayoutDate'] = 'Payment date:';
$strings['CautionThisProcessCantBeCanceled'] = '<b>Caution</b>: this process can take a few minutes and cannot be cancelled.';
$strings['ProcessingPayoutsDontCloseThisWindow'] = "<b>Proceeding with payments. Don't close this window until this process is finalized.</b>";
$strings['PayoutSuccess'] = 'Payments processed successfully';
$strings['Buyer'] = 'Buyer';
$strings['PurchaseDate'] = 'Purchase date';
$strings['BankTransfer'] = 'Bank transfer';
$strings['SaleInfo'] = 'Sale information';
$strings['SaleStatusPending'] = 'Sale pending';
$strings['SaleStatusCanceled'] = 'Sale canceled';
$strings['SaleStatusCompleted'] = 'Sale completed';
$strings['PayoutStatusPending'] = 'Payment pending';
$strings['PayoutStatusCanceled'] = 'Payment cancelled';
$strings['PayoutStatusCompleted'] = 'Payment completed';
$strings['PayoutsTotalPending'] = 'Pending payments:';
$strings['PayoutsTotalCanceled'] = 'Cancelled payments:';
$strings['PayoutsTotalCompleted'] = 'Completed payments:';
$strings['Total'] = 'Total';
$strings['TotalAmount'] = 'Total amount:';
$strings['CourseListOnSale'] = 'List of courses on sale';
$strings['AvailableCourses'] = 'Available Courses';
$strings['Price'] = 'Price';
$strings['SearchFilter'] = 'Search filter';
$strings['MinimumPrice'] = 'Minimum price';
$strings['MaximumPrice'] = 'Maximum price';
$strings['AvailableCoursesConfiguration'] = 'Available courses configuration';
$strings['PaymentsConfiguration'] = 'Payments configuration';
$strings['TheUserIsAlreadyRegisteredInTheCourse'] = 'You are already registered in the course.';
$strings['SeeDescription'] = 'Description';
$strings['Buy'] = 'Buy';
$strings['WaitingToReceiveThePayment'] = 'Currently pending payment';
$strings['TheUserIsAlreadyRegisteredInTheSession'] = 'You are already registered in the session';
$strings['ItemNotSaved'] = 'Item not saved';
$strings['TitlePlugin'] = 'Everything you need to teach and sell courses online';
$strings['PluginPresentation'] = 'The BuyCourses Plugin gives you the means to sell your courses or sessions and teach online, through only a few simple steps and settings. What are you waiting for? Start selling courses through Chamilo LMS!';
$strings['Instructions'] = 'Instructions';
$strings['InstructionsStepOne'] = 'Create a course or session on the platform.';
$strings['InstructionsStepTwo'] = 'In the <strong>payment settings</strong>, set the currency with which you would like to sell your courses or sessions.';
$strings['InstructionsStepThree'] = 'Configure courses to sell in the <strong>Courses and prices</strong > settings.';
$strings['BuyCourses'] = 'Buy courses';
$strings['ConfigurationOfCoursesAndPrices'] = 'Courses and prices configuration';
$strings['SalesReport'] = 'Sales report';
$strings['UserInformation'] = "Buyer's details";
$strings['PaymentMethods'] = 'Payment methods';
$strings['ConfirmOrder'] = 'Confirm order';
$strings['PurchaseData'] = 'Purchase data';
$strings['bc_subject'] = 'Confirmation of course order';
$strings['PurchaseStatusX'] = 'Purchase status: %s';
$strings['PendingReasonByTransfer'] = '<b>Pending</b>. Awaiting for transfer confirmation';
$strings['CancelOrder'] = 'Cancel order';
$strings['BankAccountInformation'] = 'Bank account details';
$strings['BankAccount'] = 'Bank account';
$strings['OnceItIsConfirmedYouWillReceiveAnEmailWithTheBankInformationAndAnOrderReference'] = 'Once confirmed, you will receive an e-mail with the bank details and an order reference.';
$strings['SubscriptionToCourseXSuccessful'] = 'Completed process. Your subscription to <a href="%s"><strong>"%s"</strong></a> was completed successfully.';
$strings['OrderCanceled'] = 'Order canceled';
$strings['OrderStatus'] = 'Order status';
$strings['PayoutStatus'] = 'Payment status';
$strings['SearchByStatus'] = 'Search by status';
$strings['OrderReference'] = 'Order reference';
$strings['OrderDate'] = 'Order date';
$strings['OrderPrice'] = 'Order price';
$strings['ProductType'] = 'Product type';
$strings['SubscribeUser'] = 'Subscribe user';
$strings['DeleteOrder'] = 'Delete order';
$strings['ErrorContactPlatformAdmin'] = 'Unknown error. Please contact the platform administrator.';
$strings['PendingReasonByAddress'] = '<b>Pending</b>. You did not enter a confirmed shipping address.';
$strings['PendingReasonByAuthorization'] = '<b>Pending</b>. We have not yet captured funds.';
$strings['PendingReasonByEcheck'] = '<b>Pending</b>. The payment was made by an eCheck that has not yet cleared';
$strings['PendingReasonByIntl'] = '<b>Pending</b>. We have no a withdrawal mechanism for funds.';
$strings['PendingReasonByMulticurrency'] = '<b>Pending</b>. We have not balance in the currency sent.';
$strings['PendingReasonByOrder'] = '<b>Pending</b>. Order made. We have not yet captured funds.';
$strings['PendingReasonByPaymentReview'] = '<b>Pending</b>. The payment is being reviewed by PayPal for risk.';
$strings['PendingReasonByRegulatoryReview'] = '<b>Pending</b>. The payment is being reviewed for compliance with government regulations.';
$strings['PendingReasonByUnilateral'] = '<b>Pending</b>. The email address is not yet registered o confirmed.';
$strings['PendingReasonByUpgrade'] = '<b>Pending</b>. The payment was made via credit card.';
$strings['PendingReasonByVerify'] = '<b>Pending</b>. Sorry. We are not yet verified in PayPal.';
$strings['PendingReasonByOther'] = '<b>Pending</b>. Please contact with the platform admin.';
$strings['PayPalPaymentOKPleaseConfirm'] = "PayPal reports the transaction is ready to be executed. To acknowledge that you are OK to proceed, please click the confirmation button below. Once clicked, you will be registered to the course and the funds will be transferred from your PayPal account to our shop. You can always access your courses through the 'My courses' tab. Thank you for your custom!";
$strings['Sandbox'] = 'Test environment';
$strings['PayPalAccount'] = 'Paypal account';
$strings['NoPayPalAccountDetected'] = 'No Paypal account detected';
$strings['PayPalConfig'] = 'PayPal configuration:';
$strings['TransfersConfig'] = 'Bank transfers configuration:';
$strings['PluginInstruction'] = 'You can enable or disable the option to pay via PayPal, Culqi or bank transfer settings in the configuration section of the plugin.';
$strings['ClickHere'] = '<i>Click here</i> for more details';
$strings['CurrencyType'] = 'Currency type';
$strings['InfoCurrency'] = 'Sets the currency for the payment of your courses.';
$strings['ApiUsername'] = 'API Username';
$strings['ApiPassword'] = 'API Password';
$strings['ApiSignature'] = 'API Signature';
$strings['InfoApiCredentials'] = 'To generate your API credentials to integrate Chamilo with your PayPal account, you must follow the following steps';
$strings['InfoApiStepOne'] = 'Go to your PayPal account, <strong>Summary</strong>, then <front>Seller preferences</front> in the <front>Selling Tools</front> menu (if you do not have this menu entry, you might need to get the authorization to sell stuff through PayPal first).';
$strings['InfoApiStepTwo'] = 'In paragraph <strong>API access</strong>, click <strong>Update</strong>';
$strings['InfoApiStepThree'] = 'In Option 2 (Request API credentials to create your own API username and password), click the <strong>View API Signature</strong> link, and copy the credentials shown right into the BuyCourses plugin form';
$strings['ErrorOccurred'] = '<strong>An error ocurred</strong>. Code: %s. Message: %s. Please contact to platform admin';
$strings['VisibleInCatalog'] = 'Visible in catalog';
$strings['ActiveService'] = 'Active';
$strings['ActiveServiceHelp'] = 'When disabled, the service remains available for administration and existing purchases, but it is hidden from sales and cannot be purchased or used as a new upgrade.';
$strings['Beneficiaries'] = 'Beneficiaries';
$strings['AvailableCourse'] = 'Available course';
$strings['ShowOnCourseCatalog'] = 'Show on course catalog';
$strings['ByStatus'] = 'By status';
$strings['ByUser'] = 'By user';
$strings['ByEmail'] = 'By email';
$strings['PaymentMethod'] = 'Payment method';
$strings['SWIFT'] = 'SWIFT code';
$strings['SWIFT_help'] = 'Standard format of Bank Identifier Codes (BIC) and serves as a unique identifier for a bank or financial institution';
$strings['PleaseSelectThePaymentMethodBeforeConfirmYourOrder'] = 'Please select your favorite payment method before confirming your order';
$strings['NoPaymentOptionAvailable'] = 'No payment option available. Please report to the administrator.';
$strings['XIsOnlyPaymentMethodAvailable'] = '%s is the only payment method available for this purchase.';
$strings['hide_free_text'] = "Hide 'Free' text";
$strings['culqi_enable'] = 'Enable culqi';
$strings['include_services'] = 'Include services';
$strings['CurrencyIsNotConfigured'] = 'Please, configure a currency before continuing.';
$strings['Services'] = 'Services';
$strings['Service'] = 'Service';
$strings['NewService'] = 'New service';
$strings['ServiceName'] = 'Service name';
$strings['AppliesTo'] = 'Applies to';
$strings['ServiceInformation'] = 'Service information';
$strings['ListOfServicesOnSale'] = 'List of services on sale';
$strings['GlobalConfig'] = 'Global configuration';
$strings['WriteHereTheTermsAndConditionsOfYourECommerce'] = 'Write here the terms and conditions of your e-commerce';
$strings['EditService'] = 'Edit service';
$strings['DeleteThisService'] = 'Delete this service';
$strings['IConfirmIReadAndAcceptTermsAndCondition'] = 'I confirm I read and accept the terms and conditions';
$strings['PleaseSelectTheCorrectInfoToApplyTheService'] = 'Please select the correct info to apply the service';
$strings['SaleStatusCancelled'] = 'Sale cancelled';
$strings['ServiceSaleInfo'] = 'Service sale info';
$strings['ServiceId'] = 'Service Id';
$strings['BoughtBy'] = 'Bought by';
$strings['PurchaserUser'] = 'Purchaser user';
$strings['Pending'] = 'Pending';
$strings['Names'] = 'Names';
$strings['SellerName'] = 'Seller name';
$strings['SellerId'] = 'Seller id';
$strings['SellerAddress'] = 'Seller address';
$strings['SellerEmail'] = 'Seller e-mail';
$strings['NextNumberInvoice'] = 'Next invoice number';
$strings['NextNumberInvoiceDescription'] = 'Number of the following invoice';
$strings['InvoiceSeries'] = 'Invoice series';
$strings['InvoiceSeriesDescription'] = 'Optional parameter: Example invoice Id &lt;series&gt;&lt;year&gt;/&lt;number&gt;';
$strings['InvoiceView'] = 'View invoice';
$strings['NoInvoiceEnable'] = 'No invoicing block enable';
$strings['Company'] = 'Company';
$strings['VAT'] = 'VAT';
$strings['Address'] = 'Address';
$strings['InvoiceNumber'] = 'Invoice number';
$strings['InvoiceDate'] = 'Invoice date';
$strings['Invoice'] = 'Invoice';
$strings['ReceiptNumber'] = 'Receipt number';
$strings['PaymentId'] = 'Payment ID';
$strings['RequestVatInvoice'] = 'I want a VAT invoice for this purchase';
$strings['InvoiceRequestWindowExpired'] = 'This purchase is too old to request an invoice for it. Please contact the platform administrator.';
$strings['InvoiceNotYetAvailable'] = 'No invoice is available for this purchase yet.';
$strings['SaleEmail'] = 'Sales e-mail';
$strings['PurchaseDetailsIntro'] = 'Purchase details';
$strings['PurchaseDetailsEnd'] = 'Regards';
$strings['ProductName'] = 'Product name';
$strings['BankAccountIntro'] = 'Bank Account Info';
$strings['AdditionalInfoRequired'] = 'More information needed';
$strings['SubscriptionToServiceXSuccessful'] = 'Subscription to service %s completed.';
$strings['ClickHereToFinish'] = 'Click here to finish';
$strings['OrderCancelled'] = 'Order cancelled';
$strings['use_currency_symbol'] = 'Use currency symbol';
$strings['ExportReport'] = 'Export Sales Report';
$strings['GenerateReport'] = 'Generate report';
$strings['Export'] = 'Export';
$strings['ExportExcel'] = 'Export Excel';
$strings['DateStart'] = 'Start date';
$strings['DateEnd'] = 'End date';
$strings['OrderTime'] = 'Order time';
$strings['SelectDateRange'] = 'Select a <strong>start date</strong> and <strong>end date</strong> for the report';
$strings['ServiceAdded'] = 'Service added';
$strings['ServiceEdited'] = 'Service updated';
$strings['ListOfServicesOnSale'] = 'List of services for sale';
$strings['AdditionalInfo'] = 'Additional information';
$strings['culqi_enable'] = 'Enable Culqi';
$strings['CulqiConfig'] = 'Culqi configuration:';
$strings['InfoCulqiCredentials'] = 'To obtain your credentials, you will need to create an account on Culqi and enter the development mode, copy the merchant code in your dashboard, then enter the API Keys section and copy the corresponding key to paste it here.';
$strings['CommerceCode'] = 'Merchant code';
$strings['NoTermsAndConditionsProvided'] = 'No defined terms and conditions';
$strings['GlobalConfig'] = 'Global configuration:';
$strings['MyServices'] = 'My services';
$strings['SalePrice'] = 'Sale price';
$strings['YouNeedToBeRegisteredInAtLeastOneCourse'] = 'You need to be registered in at least one course';
$strings['YouNeedToBeRegisteredInAtLeastOneSession'] = 'You need to be registered in at least one session';
$strings['IfYouWantToGetTheCertificateAndOrSkillsAsociatedToThisCourseYouNeedToBuyTheCertificateServiceYouCanGoToServiceCatalogClickingHere'] = "To obtain the certificate and/or the skills associated to this course, you need to buy the <b> Certificate </b> service. Go to the services catalogue to buy it by clicking <a target='_blank' href='%s'>here</a>";
$strings['ServiceDeleted'] = 'Service deleted';
$strings['ServiceCopied'] = 'Service copied successfully.';
$strings['ServiceCopyFailed'] = 'The service could not be copied.';
$strings['ServiceCopyInvalidSecurityToken'] = 'The copy request is no longer valid. Please try again.';
$strings['CopyServiceLink'] = 'Copy link';
$strings['ServiceLinkCopied'] = 'Link copied';
$strings['ServiceLinkCopyFailed'] = 'The link could not be copied automatically. Please copy it manually.';
$strings['YourCoursesNeedAtLeastOneLearningPath'] = 'The courses to which you are subscribed need at least one learning path that contains a final certificate item';
$strings['GlobalTaxPerc'] = 'Global tax rate';
$strings['GlobalTaxPercDescription'] = 'Default tax rate that will be used unless there is a specific tax rate for the course, session or service.';
$strings['TaxPerc'] = 'Tax rate';
$strings['TaxPercDescription'] = 'If left blank, the global tax rate will be used.';
$strings['ByDefault'] = 'by default (global value)';
$strings['OnlyCourses'] = 'Only courses';
$strings['OnlySessions'] = 'Only sessions';
$strings['OnlyServices'] = 'Only services';
$strings['TaxAppliesTo'] = 'Tax applied to';
$strings['AllCoursesSessionsAndServices'] = 'All (courses, sessions and services)';
$strings['TaxNameCustom'] = 'Tax name';
$strings['TaxNameExamples'] = 'VAT, IVA, IGV, TVA, IV ...';
$strings['ErrorUpdateFieldDB'] = 'Error updating the database fields';
$strings['tpv_redsys_enable'] = 'Enable RedSys POS';
$strings['tpv_redsys_enable_help'] = "In order to use the RedSys POS payment method, it is necessary to download the \"REST INTEGRATION - PHP API \" files at the following link <a href='https://pagosonline.redsys.es/descargas.html'>web de RedSys</a> and locate the file <strong>apiRedSys.php</strong> in the <em>plugin/BuyCourses/resources</em> directory.";
$strings['NotFindRedsysFile'] = 'The <strong>apiRedsys.php</strong> file cannot be found in <em>plugin/BuyCourses/resources</em> directory';
$strings['TpvPayment'] = 'POS payment';
$strings['TpvRedsysConfig'] = 'Redsys POS configuration';
$strings['DS_MERCHANT_MERCHANTCODE'] = 'Trade number (FUC)';
$strings['DS_MERCHANT_TERMINAL'] = 'Terminal number';
$strings['DS_MERCHANT_CURRENCY'] = 'Terminal currency';
$strings['kc'] = 'Secret encryption key';
$strings['url_redsys'] = 'Redsys connection URL';
$strings['url_redsys_sandbox'] = 'Redsys connection URL (Sandbox)';
$strings['InfoTpvRedsysApiCredentials'] = 'You must complete the following form fields with the information provided by the Redsys POS Technical Support:';
$strings['InfoEmailExtra'] = 'Extra info in payment e-mail';
$strings['Coupon'] = 'Coupon';
$strings['DiscountAmount'] = 'Discount amount';
$strings['RedeemCoupon'] = 'Redeem';
$strings['NeedToAddCouponCode'] = 'A coupon code has not been entered';
$strings['CouponNotValid'] = 'This coupon is not valid';
$strings['CouponRedeemed'] = 'Coupon redeemed';
$strings['CouponDiscount'] = 'Discount';
$strings['CouponStatus'] = 'Coupon status';
$strings['CouponPercentage'] = 'Percentage';
$strings['CouponAmount'] = 'Amount';
$strings['CouponList'] = 'Coupon list';
$strings['CouponCode'] = 'Coupon code';
$strings['CouponDiscountType'] = 'Discount type';
$strings['CouponDateStart'] = 'Valid from';
$strings['CouponDateEnd'] = 'Expires on';
$strings['CouponDelivered'] = 'Delivered';
$strings['CouponDisable'] = 'Disable';
$strings['CouponEnable'] = 'Enable';
$strings['CouponCodeUsed'] = 'Coupon code already used';
$strings['CouponNoExists'] = 'Coupon no exists';
$strings['CouponErrorInsert'] = 'Error inserting new coupon';
$strings['CouponActive'] = 'Active';
$strings['CouponDisabled'] = 'Disabled';
$strings['CouponUpdate'] = 'Coupon updated';
$strings['CouponsConfiguration'] = 'Coupons configuration';
$strings['CouponAdd'] = 'New coupon';
$strings['ConfigureCoupon'] = 'Configure coupon';
$strings['DiscountCoupon'] = 'Discount coupons';
$strings['CouponsCode'] = 'Code';
$strings['DoYouHaveACoupon'] = 'Do you have a coupon?';
$strings['stripe_enable'] = 'Enable Stripe';
$strings['StripeConfig'] = 'Stripe configuration:';
$strings['InfoStripeCredentials'] = 'To obtain the credentials you must first create an account in Stripe, copy the account id from your profile, go to the API Keys configuration section and copy the secret key, finally you must go to the Developers button (top right), register a new Endpoint in the Webhooks section, pointing to https://{site}/plugin/BuyCourses/src/stripe_response.php for the checkout.session.completed event type and copy the secret of the Endpoint.';
$strings['StripeAccountId'] = 'Account id:';
$strings['StripeSecret'] = 'Secret key:';
$strings['StripeEndpointSecret'] = 'Endpoint secret:';
$strings['PendingReasonByStripe'] = '<b>Pending</b>. Awaiting for payment confirmation ...';
$strings['cecabank_enable'] = 'Enable Cecabank POS';
$strings['TpvCecabank'] = 'POS payment (Cecabank)';
$strings['CecaSecret'] = 'Secret';
$strings['CecaUrl'] = 'Url';
$strings['CecaMerchanId'] = 'Merchant';
$strings['CecaAcquirerId'] = 'Adquirer';
$strings['CecaTerminalId'] = 'Terminal';
$strings['CecaCypher'] = 'Cypher';
$strings['CecaCurrency'] = 'Currency';
$strings['CecaExponent'] = 'Exponent';
$strings['CecaSupportedPayment'] = 'Payment supported';
$strings['CecabankConfig'] = 'Cecabank configuration';
$strings['Country'] = 'Country';
$strings['PaymentType'] = 'Payment type';
$strings['CountryRelPaymentConfig'] = 'Payment type configuration for the country';
$strings['CountryRelPaymentMessage'] = 'To process an order, the type of payment per country must be defined, otherwise the order will not be allowed to go through.';
$strings['CountryEmpty'] = 'To process an order, you need to define the Country field in the user profile.';
$strings['Duration'] = 'Duration';
$strings['SubscriptionAdd'] = 'Add subscription';
$strings['SubscriptionList'] = 'Subscription list';
$strings['SubscriptionListOnSale'] = 'Subscriptions on sale';
$strings['SelectSubscription'] = 'Select duration';
$strings['SubscriptionNotValid'] = 'Subscription not valid';
$strings['SubscriptionSalesReport'] = 'Sales report';
$strings['BuySubscriptions'] = 'Buy subscriptions';
$strings['ConfigurationOfSubscriptionsAndPrices'] = 'Subscriptions and prices configuration';
$strings['FrequencyConfig'] = 'Duration config';
$strings['Subscriptions'] = 'Subscriptions';
$strings['HasSubscriptions'] = 'Has subscriptions';
$strings['FrequencyRemoved'] = 'Period removed';
$strings['SubscriptionPeriodOnUse'] = 'Subscription period in use';
$strings['FrequencyNotExits'] = 'Period does not exist';
$strings['FrequencyIncorrect'] = 'Period incorrect';
$strings['SubscriptionFrequencyValueDays'] = 'Value on days';
$strings['FrequencyNotUpdated'] = 'Period not updated';
$strings['FrequencyNotSaved'] = 'Period not saved';
$strings['NeedToAddDuration'] = 'Need to add duration';
$strings['SubscriptionNotValid'] = 'Subscription not valid';
$strings['SelecSubscription'] = 'Select a subscription';
$strings['ConfigureSubscriptionsFrequencies'] = 'Configure subscriptions periods';
$strings['FrequencyAdd'] = 'Add periods';
$strings['SubscriptionAlreadyExists'] = 'Subscription already exists';
$strings['SubscriptionPeriodDuration'] = 'Subscription duration (in days)';
$strings['Product'] = 'Product name';
$strings['YouProductIsActivatedYouCanNowAccessIt'] = 'Your product is now activated and you can now have access to it.';
$strings['hide_shopping_cart_from_course_catalogue'] = 'Hide shopping cart from the course catalogue and leave the subscribe button';
$strings['Actions'] = 'Actions';
$strings['Active'] = 'Active';
$strings['ActivePaymentMethods'] = 'Active payment methods';
$strings['AddPeriods'] = 'Add periods';
$strings['AddPeriodsHelp'] = 'Create or update a subscription period by setting a name and its duration in days.';
$strings['AdminPermissionsRequired'] = 'You must have admin permissions to install plugins.';
$strings['Available Courses'] = 'Available courses';
$strings['Available course'] = 'Available course';
$strings['BankAccounts'] = 'Bank accounts';
$strings['ByDate'] = 'By date';
$strings['Cancelled'] = 'Cancelled';
$strings['CatalogCoursesFilterHelp'] = 'Filter the catalog by course name, minimum price and maximum price.';
$strings['CatalogCoursesIntro'] = 'Browse the courses currently available for sale. Use the filters to search by course name and price range.';
$strings['CatalogServicesFilterHelp'] = 'Filter the catalog by service name, minimum price and maximum price.';
$strings['CatalogServicesIntro'] = 'Browse the services currently available for sale. Use the filters to search by name and price range.';
$strings['ServiceDurationXDays'] = '%s days';
$strings['YearlyPlans'] = 'Yearly';
$strings['MonthlyPlans'] = 'Monthly';
$strings['YearlyPlan'] = 'Yearly';
$strings['MonthlyPlan'] = 'Monthly';
$strings['BillingPeriod'] = 'Billing period';
$strings['CatalogSessionsFilterHelp'] = 'Filter the catalog by session name, minimum price and maximum price.';
$strings['CatalogSessionsIntro'] = 'Browse the sessions currently available for sale. Use the filters to search by session name and price range.';
$strings['ConfigureFrequencyIntro'] = 'Define the subscription periods that will be available when creating or updating course and session subscriptions.';
$strings['ConfiguredPeriods'] = 'Configured periods';
$strings['ConfiguredPeriodsHelp'] = 'Delete a period only if it is no longer being used by any subscription.';
$strings['CouponDiscountExceed100'] = 'The coupon discount cannot be greater than 100%.';
$strings['CouponsFilterHelp'] = 'Filter the coupon list by activation status.';
$strings['CouponsScopeHelp'] = 'Use this page to review active or disabled coupons and open them for configuration.';
$strings['CourseDescriptionUnavailable'] = 'No description is available for this course yet.';
$strings['Days'] = 'Days';
$strings['Description'] = 'Description';
$strings['DurationUnit'] = 'Duration unit';
$strings['EnterNameToSearch'] = 'Enter a name to search';
$strings['ExportReportFormHelp'] = 'Choose the sales source, status and date range. You can also search by reference, product, user, email or coupon.';
$strings['ExportReportIntro'] = 'Filter the BuyCourses sales report by source, status, date range or search text before exporting it to Excel.';
$strings['ExportReportUsageHelp'] = 'Export the sales currently returned by the selected filters.';
$strings['FrequenciesNotSetError'] = 'You must configure at least one subscription frequency.';
$strings['FrequencyUsageHelp'] = 'Reused by subscription forms for courses and sessions.';
$strings['GatewayAvailability'] = 'Gateway availability';
$strings['LandingBrowseIntro'] = 'Browse the available products and continue to checkout.';
$strings['LandingChooseWhatToBuy'] = 'Choose what you want to buy';
$strings['NoBankAccountsConfiguredYet'] = 'No bank accounts configured yet.';
$strings['NoCouponsFoundForStatus'] = 'No coupons were found for the selected status.';
$strings['NoCoursesFound'] = 'No courses found.';
$strings['NoPaymentMethodsEnabledYet'] = 'No payment methods enabled yet.';
$strings['NoServicesFound'] = 'No services found.';
$strings['NoSessionsFound'] = 'No sessions found.';
$strings['NoSubscriptionsFound'] = 'No subscriptions found.';
$strings['NoTeacherInformationAvailable'] = 'No teacher information available.';
$strings['OpenPluginSettings'] = 'Open plugin settings';
$strings['PayPalApiCredentialsIncomplete'] = 'PayPal API credentials are incomplete.';
$strings['PayPalErrorCodeMessage'] = 'PayPal error: %s - %s';
$strings['PluginSettings'] = 'Plugin settings';
$strings['ProcessingDontCloseThisWindow'] = 'Processing. Do not close this window.';
$strings['SalesReportSearchHelp'] = 'Choose a filter mode and update the fields below to refresh the sales report.';
$strings['ServiceSalesReportIntro'] = 'Service sales report with optional user-based narrowing.';
$strings['SessionListOnSale'] = 'List of sessions on sale';
$strings['Sessions'] = 'Sessions';
$strings['Status'] = 'Status';
$strings['StripeUnknownEventTypeX'] = 'Received unknown event type: %s';
$strings['SubscriptionCatalogIntro'] = 'Browse the subscriptions currently available for sale and filter them by name.';
$strings['SubscriptionCatalogSearchHelp'] = 'Search subscriptions by course or session name.';
$strings['SubscriptionErrorInsert'] = 'The subscription could not be saved.';
$strings['SubscriptionSalesReportSearchHelp'] = 'Choose a filter mode and update the fields below to refresh the subscription sales report.';
$strings['TabsDashboard'] = 'Dashboard';
$strings['TaxSettings'] = 'Tax settings';
$strings['TransferEmailInformation'] = 'Transfer email information';
$strings['TransferEmailInformationHelp'] = 'Extra information shown in the email sent after selecting bank transfer.';
$strings['TryChangingSearchFilter'] = 'Try changing the search filter.';
$strings['UnknownPayPalError'] = 'Unknown PayPal error';
$strings['Updated'] = 'Updated';
$strings['Usage'] = 'Usage';
$strings['UserName'] = 'Username';
$strings['UserPanel'] = 'User panel';
$strings['SetExpressCheckoutApiCallFailed'] = 'SetExpressCheckout API call failed. ';
$strings['DetailedErrorMessageX'] = 'Detailed error message: %s';
$strings['ShortErrorMessageX'] = 'Short error message: %s';
$strings['ErrorCodeX'] = 'Error code: %s';
$strings['ErrorSeverityCodeX'] = 'Error severity code: %s';
$strings['Coupons'] = 'Coupons';
$strings['DeleteFrequency'] = 'Delete period';
$strings['Invoicing'] = 'Invoicing';
$strings['Teachers'] = 'Teachers';
$strings['TermsAndConditions'] = 'Terms and conditions';


$strings['GrantedBenefits'] = 'Granted benefits';
$strings['BenefitMaxCoursesTitle'] = 'Number of courses with these benefits';
$strings['BenefitMaxCoursesDescription'] = 'Number of courses that can use these service benefits while the service is valid.';
$strings['BenefitHostingLimitTitle'] = 'Hosting limit per course';
$strings['BenefitHostingLimitDescription'] = 'Maximum number of students allowed in the buyer\'s courses while the service is active.';
$strings['BenefitDocumentQuotaTitle'] = 'Document quota';
$strings['BenefitDocumentQuotaDescription'] = 'Maximum document quota granted to the buyer\'s courses while the service is active.';
$strings['BenefitCoursesUnit'] = 'courses';
$strings['BenefitUsersUnit'] = 'users';
$strings['BenefitMegabytesUnit'] = 'MB';
$strings['BenefitSummaryValueUntil'] = '%s until %s';
$strings['Receipt'] = 'Receipt';
$strings['PurchaseHistory'] = 'Purchase history';
$strings['PurchaseHistoryDescription'] = 'Review your completed purchases and download the related receipts when available.';
$strings['ActiveServices'] = 'Active services';
$strings['ActiveServicesDescription'] = 'These services are still valid and currently applied to your account.';
$strings['NoActiveServicesYet'] = 'You do not have active services yet.';
$strings['NoPurchasesYet'] = 'You do not have purchases yet.';
$strings['BackToServices'] = 'Back to services';
$strings['QuickSummary'] = 'Quick summary';
$strings['ServiceCurrencyLabel'] = 'Currency';
$strings['DefaultTaxLabel'] = 'Default tax';
$strings['ImageCropLabel'] = 'Image crop';
$strings['CurrentImageLabel'] = 'Current image';
$strings['ServiceFormTitle'] = 'Service form';
$strings['ServiceFormSubtitle'] = 'Fill in the basic information, price, visibility, and optional media.';
$strings['ServiceAddSubtitle'] = 'Create a service, define pricing, and choose who it applies to.';
$strings['ServiceEditSubtitle'] = 'Update service details, pricing, media, and catalog visibility.';
$strings['ServiceGeneralSectionTitle'] = 'General information';
$strings['ServiceGeneralSectionHelp'] = 'Define the service name and the multilingual description shown to users.';
$strings['ServicePricingSectionTitle'] = 'Pricing and duration';
$strings['ServicePricingSectionHelp'] = 'Set the price, tax behavior, duration, and optional upgrade source.';
$strings['ServiceRecurringSectionHelp'] = 'Configure automatic renewal, trial conditions, subscriber limits, and gateway references.';
$strings['ServicePublishingSectionTitle'] = 'Publication and ownership';
$strings['ServicePublishingSectionHelp'] = 'Control where the service appears and who owns or manages it.';
$strings['ServiceMediaSectionTitle'] = 'Media and presentation';
$strings['ServiceMediaSectionHelp'] = 'Add the service image, video URL, and detailed multilingual information.';
$strings['ServiceBenefitsSectionHelp'] = 'Define the limits and AI capabilities granted while the service remains active.';
$strings['ServiceDangerZoneTitle'] = 'Danger zone';
$strings['ServiceDangerZoneHelp'] = 'Deleting a service is irreversible. Confirm that it is no longer needed before continuing.';
$strings['ServiceTypeFixedToUser'] = 'This service type is fixed to user level in the current implementation.';
$strings['GrantedBenefitsOnlyForUserServices'] = 'Granted benefits are only available for services that apply to User.';
$strings['DurationMustBePositiveForBenefits'] = 'Duration must be greater than 0 when the service grants benefits.';
$strings['AiCourseFeaturesGranted'] = 'AI features enabled in created courses';
$strings['AiCourseFeaturesGrantedHelp'] = 'Selected AI tools will be enabled only in courses created with this service while it is active.';
$strings['AiCourseFeatureLearningPathGeneratorTitle'] = 'Learning path generator';
$strings['AiCourseFeatureLearningPathGeneratorDescription'] = 'Allow teachers to generate learning paths with AI.';
$strings['AiCourseFeatureExerciseGeneratorTitle'] = 'Exercise generator';
$strings['AiCourseFeatureExerciseGeneratorDescription'] = 'Allow teachers to generate Aiken questions with AI.';
$strings['AiCourseFeatureOpenAnswersGraderTitle'] = 'Open answers grader';
$strings['AiCourseFeatureOpenAnswersGraderDescription'] = 'Allow teachers to grade open answers with AI assistance.';
$strings['AiCourseFeatureTutorChatbotTitle'] = 'Tutor chatbot';
$strings['AiCourseFeatureTutorChatbotDescription'] = 'Allow the course AI tutor chatbot.';
$strings['AiCourseFeatureTaskGraderTitle'] = 'Task grader';
$strings['AiCourseFeatureTaskGraderDescription'] = 'Allow AI grading support for assignments.';
$strings['AiCourseFeatureContentAnalyserTitle'] = 'Content analyser';
$strings['AiCourseFeatureContentAnalyserDescription'] = 'Allow AI analysis and feedback for course documents.';
$strings['AiCourseFeatureImageGeneratorTitle'] = 'Image generator';
$strings['AiCourseFeatureImageGeneratorDescription'] = 'Allow teachers to generate images with AI.';
$strings['AiCourseFeatureGlossaryTermsGeneratorTitle'] = 'Glossary terms generator';
$strings['AiCourseFeatureGlossaryTermsGeneratorDescription'] = 'Allow teachers to generate glossary terms with AI.';
$strings['AiCourseFeatureCourseAnalyserTitle'] = 'Course analyser';
$strings['AiCourseFeatureCourseAnalyserDescription'] = 'Allow teachers to analyse the course with AI.';
$strings['AiCourseFeatureVideoGeneratorTitle'] = 'Video generator';
$strings['AiCourseFeatureVideoGeneratorDescription'] = 'Allow teachers to generate videos with AI.';
$strings['AiCourseFeatureVideoWarning'] = 'This feature can consume many tokens; keep it disabled unless the plan explicitly includes it.';
$strings['PayPalCredentialsIncomplete'] = 'PayPal credentials are incomplete.';
$strings['PayPalPaymentNotConfirmed'] = 'PayPal payment could not be confirmed.';
$strings['PayPalUnexpectedStatus'] = 'Unexpected PayPal payment status.';
$strings['ServicesOnlyForTeachers'] = 'These services are available only for teachers.';
$strings['PaymentSetupIntro'] = 'Configure the global e-commerce settings, choose the active payment methods, and update the credentials required by each gateway.';
$strings['GeneralSettingsHelp'] = 'Configure currency, terms and conditions, sales contact email, tax rules, and invoicing data.';
$strings['PayPalSettingsHelp'] = 'Configure the API credentials required to process payments through PayPal.';
$strings['RedsysSettingsHelp'] = 'Configure the Redsys terminal credentials and sandbox settings.';
$strings['CommissionsSettingsHelp'] = 'Set the platform commission percentage used by the marketplace.';
$strings['BankTransferSettingsHelp'] = 'Manage bank accounts for manual transfers and the extra message sent by email after a transfer purchase.';
$strings['TransferEmailHelp'] = 'Extra instructions appended to the email sent after choosing bank transfer.';
$strings['CulqiSettingsHelp'] = 'Configure the API credentials required to process payments through Culqi.';
$strings['StripeSettingsHelp'] = 'Configure your Stripe account ID, secret key and webhook endpoint secret.';
$strings['CecabankSettingsHelp'] = 'Configure the keys and merchant identifiers required by Cecabank.';
$strings['SalesReportIntro'] = 'Review course and session sales, filter the report by status, user, date or email, and export the current report when needed.';
$strings['SalesReportTableIntro'] = 'Interactive report generated from the current filter.';
$strings['ServiceSalesReportIntroFull'] = 'Review service sales, filter them by status, and optionally narrow the results by user, email, service name or reference.';
$strings['ServiceSalesFilterHelp'] = 'Filter by service order status and search by user, email, service name or order reference.';
$strings['ServiceSalesTableIntro'] = 'Review the sales returned by the current filters.';
$strings['NoServiceSalesFound'] = 'No service sales were found for the selected filters.';
$strings['NoDetailsAvailable'] = 'No details available.';
$strings['SubscriptionSalesReportIntroFull'] = 'Review subscription sales, filter the report by status, user, date or email, and manage pending subscription orders.';
$strings['SubscriptionSalesTableIntro'] = 'Review the sales returned by the current filter and process pending subscription orders.';
$strings['NoSubscriptionSalesFound'] = 'No subscription sales were found for the selected filter.';
$strings['CouponsPageIntro'] = 'Manage discount coupons, filter them by status, and open each coupon to configure or update it.';
$strings['SubscriptionAddIntro'] = 'Create a subscription for this product, define the optional tax rate, and add one or more subscription periods with their prices.';
$strings['SubscriptionReadyToConfigure'] = 'Ready to configure subscription periods.';
$strings['SubscriptionCurrencyMissing'] = 'Currency must be configured before saving this subscription.';
$strings['SubscriptionPeriodsMissing'] = 'Subscription periods must be configured before saving this subscription.';
$strings['ConfigureSubscriptionIntro'] = 'Configure tax and subscription periods for this product.';
$strings['UpdateTaxRateHelp'] = 'Update the tax rate applied to this subscription product.';
$strings['ManagePeriodsHelp'] = 'Add or remove subscription periods for this product.';
$strings['SubscriptionsPageIntro'] = 'Configure which products offer subscriptions and open each item to manage periods and availability.';
$strings['ServiceInfoOverview'] = 'Overview';
$strings['ServiceInfoOverviewHelp'] = 'Review the main service details before continuing with the purchase.';
$strings['ServiceInfoPriceLabel'] = 'Price';
$strings['ServicePricePlusTax'] = '%s + tax';
$strings['ServiceInfoDurationLabel'] = 'Duration';
$strings['ServiceInfoOwnerLabel'] = 'Owner';
$strings['ServiceInfoPurchaseReadiness'] = 'Purchase readiness';
$strings['ServiceAvailableForPurchase'] = 'The service is available for purchase.';
$strings['ServiceCouponApplicable'] = 'You can apply a coupon before confirming the order.';
$strings['ServicePaymentMethodNextStep'] = 'The selected payment method will be configured in the next step.';
$strings['ServiceNoAdditionalInfo'] = 'No additional information is available for this service yet.';
$strings['ServiceSummaryTitle'] = 'Summary';
$strings['ServiceSummaryHelp'] = 'Review the service details before continuing with the purchase.';
$strings['ServiceTotalLabel'] = 'Total';
$strings['ShareOnFacebook'] = 'Facebook';
$strings['ShareOnTwitter'] = 'Twitter';
$strings['ShareOnLinkedin'] = 'LinkedIn';
$strings['OpenVideo'] = 'Open video';
$strings['ProcessIntro'] = 'Review the product details, apply a coupon if you have one, and confirm the payment method to continue.';
$strings['CouponCodeHelp'] = 'Enter your code to update the total before confirming the order.';
$strings['PaymentMethodHelp'] = 'Select the available method and confirm the order to continue to the next step.';
$strings['ServiceProcessIntro'] = 'Review the service details, apply a coupon if you have one, choose the payment method, and confirm the order.';
$strings['ServiceCouponCodeHelp'] = 'Add a valid coupon code before confirming the purchase.';
$strings['ServicePaymentMethodHelp'] = 'Choose the payment method and complete the required information to apply the service correctly.';
$strings['ServiceProcessSummaryTitle'] = 'Summary';
$strings['ServiceProcessSummaryHelp'] = 'Check the final amount and service conditions before placing the order.';
$strings['SubscriptionFrequencyHelp'] = 'Choose the frequency that best fits this subscription.';
$strings['SubscriptionCouponCodeHelp'] = 'Enter your coupon code to update the subscription total before paying.';
$strings['ProcessConfirmIntro'] = 'Review the purchase details and confirm the order.';
$strings['BankAccountReviewHelp'] = 'Review the bank account details before confirming the order.';
$strings['NoBankAccountsConfigured'] = 'No bank accounts are configured yet.';
$strings['TermsReviewHelp'] = 'Review and accept the terms before placing the order.';
$strings['ConfirmFinalDetails'] = 'Confirm the final details before continuing.';
$strings['BankTransferPendingNotice'] = 'Bank transfer orders remain pending until payment is verified.';
$strings['CardPaymentNextStepNotice'] = 'You will be prompted to complete the card payment in the next step.';
$strings['CecabankRedirectNotice'] = 'You will be redirected to Cecabank to complete the payment.';
$strings['SubscriptionProcessConfirmIntro'] = 'Review the order details and confirm the operation to continue with the selected payment method.';
$strings['SubscriptionConfirmOrderHelp'] = 'Confirm the order to continue with the selected payment flow.';
$strings['ManagePremiumServicesAdmin'] = 'Manage premium services and granted benefits';
$strings['BrowsePremiumServicesUser'] = 'Browse premium services and granted benefits';
$strings['FrequencyInUse'] = 'In use (%s)';
$strings['AvailableItems'] = 'Available items';
$strings['SelectedItems'] = 'Selected items';
$strings['ExcelFormat'] = 'Excel';

$strings['SubscriptionPackage'] = 'Subscription package';
$strings['RecurringPayments'] = 'Recurring payments';
$strings['RenewableService'] = 'Renew automatically';
$strings['TotalCharges'] = 'Total charges';
$strings['TotalChargesHelp'] = 'Use 0 for unlimited recurring charges.';
$strings['AllowTrial'] = 'Allow trial period';
$strings['TrialPeriod'] = 'Trial period unit';
$strings['PeriodDay'] = 'Day';
$strings['PeriodWeek'] = 'Week';
$strings['PeriodMonth'] = 'Month';
$strings['PeriodYear'] = 'Year';
$strings['TrialFrequency'] = 'Trial frequency';
$strings['TrialFrequencyHelp'] = 'Number of period units for the trial. Example: 15 days.';
$strings['TrialTotalCharges'] = 'Trial total charges';
$strings['TrialTotalChargesHelp'] = 'Use 0 when the trial should not create trial charges.';
$strings['MaxSubscribers'] = 'Maximum subscribers';
$strings['MaxSubscribersHelp'] = 'Use 0 when there is no subscriber limit for this service.';
$strings['SubscriptionBehaviorJson'] = 'Subscription behavior JSON';
$strings['SubscriptionBehaviorJsonHelp'] = 'Optional JSON used later to define expiry behavior such as close, hide or delete courses.';
$strings['TrialFrequencyMustBePositive'] = 'Trial frequency must be greater than 0 when trial is enabled.';
$strings['TrialRequiresRenewableService'] = 'Trial can only be enabled for renewable services.';
$strings['SubscriptionBehaviorJsonInvalid'] = 'Subscription behavior JSON is invalid.';
$strings['RecurringPaymentEnabled'] = 'Enabled';
$strings['RecurringPaymentDisabled'] = 'Disabled';
$strings['RecurringPaymentSuspended'] = 'Suspended';
$strings['RecurringPaymentCancelled'] = 'Cancelled';
$strings['EnableAutoBilling'] = 'Enable auto billing';
$strings['CancelAutoBilling'] = 'Cancel auto billing';
$strings['RecurringPaymentRequiresPayPal'] = 'Automatic renewal is only available for PayPal payments.';
$strings['RecurringPaymentRequiresCompletedSale'] = 'Automatic renewal can only be configured for completed purchases.';
$strings['ServiceIsNotRenewable'] = 'This service is not configured as renewable.';
$strings['RecurringPaymentAlreadyEnabled'] = 'Automatic renewal is already enabled for this service.';
$strings['RecurringPaymentEnabledSuccessfully'] = 'Automatic renewal was enabled successfully.';
$strings['RecurringPaymentCancelledSuccessfully'] = 'Automatic renewal was cancelled successfully.';
$strings['CancelRenewalTitle'] = 'Cancel renewal';
$strings['CancelRenewal'] = 'Cancel renewal';
$strings['RestoreRenewal'] = 'Restore renewal';
$strings['RestoreRenewalConfirmation'] = 'Are you sure you want to restore automatic renewal? Stripe will charge the next renewal on %1$s.';
$strings['IChangedMyMind'] = 'I changed my mind';
$strings['NextRenewal'] = 'Next renewal';
$strings['ServiceAccessUntil'] = 'Service access until';
$strings['CancelRenewalConfirmation'] = 'Are you sure you want to cancel? If you cancel, your subscription will not be renewed on %1$s and your courses will be hidden from all and stored for %2$d months, then they will be deleted if you do not activate your plan again. You can continue using them until %3$s and export the contents in a reusable format through the Maintenance options at the top of your course.';
$strings['RenewalCancelledSuccessfully'] = 'Renewal was cancelled successfully. Your service remains active until the planned end date.';
$strings['RenewalAlreadyCancelled'] = 'Renewal is already cancelled for this service.';
$strings['RenewalRestoredSuccessfully'] = 'Renewal was restored successfully. Your service will renew automatically again.';
$strings['RenewalIsNotCancelled'] = 'Renewal is not cancelled for this service.';
$strings['RecurringPaymentAccessDenied'] = 'You are not allowed to manage this service renewal.';
$strings['RecurringPaymentInvalidRequest'] = 'The renewal request is invalid.';
$strings['CancelRenewalPostRequired'] = 'Renewal cancellation must be confirmed from the service cancellation form.';
$strings['RestoreRenewalPostRequired'] = 'Renewal restoration must be confirmed from the service restoration form.';
$strings['InvalidSecurityToken'] = 'The security token is invalid or expired. Reload the page and try again.';
$strings['RecurringPaymentIsNotEnabled'] = 'Automatic renewal is not enabled for this service.';
$strings['StripeCancellationConfigurationMissing'] = 'Stripe cancellation could not be prepared because the subscription or gateway configuration is missing.';
$strings['CancelRenewalGatewayFailed'] = 'The payment gateway could not cancel the next renewal. No local cancellation was recorded.';
$strings['PayPalRecurringProfileMissing'] = 'The PayPal recurring profile could not be found.';
$strings['CancelRenewalUnsupportedGateway'] = 'This payment gateway does not support renewal cancellation from BuyCourses.';
$strings['StripeRestoreConfigurationMissing'] = 'The renewal could not be restored in Stripe because the subscription or gateway configuration is missing.';
$strings['RestoreRenewalGatewayFailed'] = 'The payment gateway could not restore the next renewal. No local restoration was recorded.';
$strings['RestoreRenewalUnsupportedGateway'] = 'This payment gateway does not support renewal restoration from BuyCourses.';
$strings['RestoreRenewalRequiresActiveService'] = 'Renewal can only be restored while the current service period is still active.';
$strings['RenewalCannotBeRestoredAfterGatewayCancellation'] = 'The renewal cannot be restored because the gateway subscription is already fully cancelled.';

$strings['DisplayServiceOnCourseCreationPage'] = 'Show this service on the course creation page';
$strings['DisplayServiceOnCourseCreationPageHelp'] = 'If enabled, teachers who have purchased this service can select it when creating a course so the course receives the service benefits.';
$strings['StandardCourseOption'] = 'Standard course';
$strings['StandardCourseOptionDescription'] = 'Create a course using the standard platform limits.';
$strings['ServiceNotPurchasedForCourseCreation'] = 'You need to buy this service before creating a course with these benefits.';
$strings['ServiceCourseLimitReachedForCourseCreation'] = 'You have already used all courses available with this service.';
$strings['InvalidCourseCreationServiceSelection'] = 'The selected service is invalid.';
$strings['BuyCoursesServicesDisabled'] = 'BuyCourses services are disabled.';
$strings['SelectedServiceUnavailableForCourseCreation'] = 'The selected service is not available for course creation.';
$strings['SelectedServiceCourseLimitReached'] = 'You have already used the %d courses available with this service.';
$strings['StripePriceId'] = 'Stripe recurring price ID';
$strings['StripePriceIdHelp'] = 'Optional. Dynamic VAT-aware Stripe Checkout uses the calculated service sale total. Keep this field only for legacy/reference prices such as price_123.';
$strings['StripeSecretKeyMissing'] = 'Stripe secret key is not configured.';
$strings['InvalidServiceSaleAmount'] = 'Invalid service sale amount.';
$strings['StripePriceIdRequiredForRecurringService'] = 'A Stripe recurring price ID is required for this legacy Stripe flow.';
$strings['StripeCheckoutCouldNotBeInitialized'] = 'Stripe Checkout could not be initialized.';
$strings['StripeCheckoutCompletedPendingConfirmation'] = 'Stripe Checkout was completed. The service will be activated as soon as Stripe confirms the payment.';

$strings['SellerCountry'] = 'Seller country';
$strings['SellerCountryHelp'] = 'Use the ISO 3166-1 alpha-2 country code, for example BE, FR or ES.';
$strings['SellerPostcode'] = 'Seller postcode';
$strings['SellerVatNumber'] = 'Seller VAT number';
$strings['BuyerIp'] = 'Buyer IP address';
$strings['Subtotal'] = 'Subtotal';
$strings['SellerVatNumberHelp'] = 'Optional VAT number used later for EU VAT validation and invoices.';
$strings['SellerVatRegistered'] = 'Seller is VAT registered';
$strings['SellerAnnualEuTbeTurnover'] = 'Annual EU TBE turnover';
$strings['SellerAnnualEuTbeTurnoverHelp'] = 'Annual turnover for telecommunications, broadcasting and electronically supplied services in the EU. Used later for VAT treatment decisions.';
$strings['VATBuyerInformation'] = 'VAT buyer information';
$strings['VATBuyerInformationHelp'] = 'This information is stored as fiscal evidence for this purchase. VAT and GeoIP validation will be added in the next phase.';
$strings['BuyerCountry'] = 'Country of residence';
$strings['BuyerCountryRequired'] = 'Country of residence is required.';
$strings['BuyerPostcode'] = 'Postcode';
$strings['BuyerPostcodeRequired'] = 'Postcode is required.';
$strings['BuyerType'] = 'Buyer type';
$strings['BuyerTypeIndividual'] = 'Individual';
$strings['BuyerTypeBusiness'] = 'Business';
$strings['BuyerVatNumber'] = 'VAT number';
$strings['BuyerVatNumberInvalidFormat'] = 'The VAT number format is invalid.';
$strings['BuyerBusinessName'] = 'Business name';
$strings['BuyerBusinessAddress'] = 'Business address';

$strings['SellerVatConfiguration'] = 'Seller VAT configuration';
$strings['SellerVatConfigurationHelp'] = 'These seller details are used to determine VAT treatment during BuyCourses service checkout. They are required even when invoicing is disabled.';
$strings['ActiveCurrencyForPayments'] = 'Active payment currency';
$strings['ActiveCurrencyVatHelp'] = 'Stripe and PayPal use this currency when creating service sales. Verify it before testing VAT amounts in a specific currency.';

$strings['BuyerGeoIpVatEvidence'] = 'Buyer GeoIP VAT evidence';
$strings['BuyerGeoIpVatEvidenceHelp'] = 'Optional GeoIP check used to compare the buyer declared country with the buyer IP country. The buyer IP is still stored in BuyCourses service sales even when GeoIP is disabled.';
$strings['GeoIpProvider'] = 'GeoIP provider';
$strings['GeoIpProviderNone'] = 'Disabled';
$strings['GeoIpProviderMaxMindWebService'] = 'MaxMind web service';
$strings['MaxMindAccountId'] = 'MaxMind account ID';
$strings['MaxMindAccountIdHelp'] = 'Required only when MaxMind web service is selected.';
$strings['MaxMindLicenseKey'] = 'MaxMind license key';
$strings['MaxMindLicenseKeyHelp'] = 'Required only when MaxMind web service is selected. Keep this value private.';

$strings['NoProductsAvailable'] = 'No products are available for purchase right now.';
$strings['EnableCourse'] = 'Enable course';
$strings['EnableSession'] = 'Enable session';
$strings['EnableSubscription'] = 'Enable subscription';

// Translation audit additions.
$strings['CancelledAction'] = 'Action cancelled.';
$strings['Name'] = 'Name';
$strings['NoPurchaseHistoryYet'] = 'No purchase history yet.';
$strings['SubscriptionNotDeleted'] = 'The subscription could not be deleted.';
$strings['SubscriptionNotUpdated'] = 'The subscription could not be updated.';
$strings['ServiceUpdateFailed'] = 'Service update failed.';
$strings['ServiceAppliedToUserAccountHelp'] = 'This service will be applied to your account.';
$strings['ServiceAppliesToReadOnlyHelp'] = 'This value is read-only in the current implementation to preserve legacy services.';
$strings['SubscriptionPeriodsNotConfigured'] = 'Subscription periods are not configured';
$strings['SubscriptionPeriodInUseCannotBeDeleted'] = 'This subscription period is currently in use and cannot be deleted.';
$strings['ConfirmRemoveSubscriptionPeriod'] = 'Are you sure you want to remove this subscription period?';
$strings['PurchasedService'] = 'Purchased';
$strings['NextCharge'] = 'Next charge';
$strings['RecurringProfile'] = 'Profile';
$strings['CancelAutoBillingConfirm'] = 'Cancel auto billing for this service?';
$strings['RecurringPaymentReplacedByNewProfile'] = 'Replaced by a new BuyCourses recurring profile.';
$strings['RecurringPaymentCancelledByCustomer'] = 'Cancelled by customer from BuyCourses service panel.';
$strings['InvalidFormTokenRefreshPage'] = 'Invalid form token. Please refresh the page and try again.';
$strings['CouponCodeRequired'] = 'Coupon code is required.';
$strings['CouponDiscountTypeRequired'] = 'Discount type is required.';
$strings['CouponDiscountMustBeZeroOrGreater'] = 'Discount must be zero or greater.';
$strings['CouponStartAndEndDatesRequired'] = 'Both start and end dates are required.';
$strings['CouponStartDateCannotBeLaterThanEndDate'] = 'The start date cannot be later than the end date.';
$strings['CouponCouldNotBeSaved'] = 'The coupon could not be saved.';
$strings['CouponAddPageDescription'] = 'Create a coupon, define the discount type and validity period, and assign it to courses, sessions or services.';
$strings['CouponFormHelp'] = 'Complete the form below and save the coupon configuration.';
$strings['CouponDiscountTypeHelp'] = 'Percentage or fixed amount.';
$strings['CouponDateHelp'] = 'Define the validity period for the coupon.';
$strings['CouponScopeHelp'] = 'Assign the coupon to one or more courses, sessions or services.';
$strings['DiscountCoupons'] = 'Discount coupons';
$strings['Recibo'] = 'Receipt';
$strings['FrequencyIsNotConfigured'] = 'Subscription periods are not configured yet. Please configure at least one subscription period before creating a subscription.';
$strings['SubscriptionSaveError'] = 'The subscription could not be saved. Please review the data and try again.';
$strings['SubscriptionPeriodsRequired'] = 'You must add at least one subscription period before saving.';
$strings['FrequencyNotRemoved'] = 'The subscription period could not be removed.';
$strings['CourseSessionBlock'] = 'Courses/Sessions';


// Service upgrade.
$strings['Upsale'] = 'Upsale';
$strings['UpsaleHelp'] = 'Select the service that this service upgrades. Leave None for a regular service.';
$strings['Upgrade'] = 'Upgrade';
$strings['ConfirmUpgrade'] = 'Confirm upgrade';
$strings['UpgradeFromService'] = 'Upgrade from %s';
$strings['UpgradeCurrentService'] = 'Current service';
$strings['UpgradeRemainingDays'] = '%s days remaining';
$strings['UpgradeProratedCredit'] = 'Prorated credit';
$strings['UpgradePriceToday'] = 'Price today';
$strings['UpgradeNextRenewalPrice'] = 'Next renewal price';
$strings['UpgradeServicePrice'] = 'Service price';
$strings['UpgradeTargetProratedPrice'] = 'Target service prorated price';
$strings['UpsaleCannotReferenceSameService'] = 'A service cannot be an upsale of itself.';
$strings['UpsaleSourceServiceNotFound'] = 'The source service selected for this upsale does not exist.';
$strings['UpsaleOnlyForUserServices'] = 'Upsale is only available between services applied to users.';
$strings['UpsaleTargetPriceMustBeHigher'] = 'The upgrade service price must be higher than the source service price.';
$strings['UpsaleRenewableModeMustMatch'] = 'Source and upgrade services must both use the same renewable mode.';
$strings['UpsaleRecurringDurationMustMatch'] = 'Renewable source and upgrade services must use the same duration.';
$strings['UpsaleTargetDurationMustNotBeShorter'] = 'The upgrade service duration must be equal to or longer than the source service duration.';
$strings['UpsaleCycleNotAllowed'] = 'This upsale relation would create a cycle between services.';
$strings['UpgradePriceMustBePositive'] = 'The calculated upgrade price must be greater than zero.';
$strings['UpgradeMustUseExistingPaymentGateway'] = 'This upgrade must use the payment gateway of the current recurring subscription.';
$strings['UpgradeSourceNoLongerAvailable'] = 'The current service is no longer available for this upgrade. Reload the catalog and try again.';
$strings['UpgradeRecurringTransitionFailed'] = 'The recurring subscription could not be moved to the upgraded service. No local service change was applied.';
$strings['UpgradeCouldNotBeCompleted'] = 'The service upgrade could not be completed. Please contact the platform administrator.';
$strings['ServiceSaleNotFound'] = 'The service sale could not be found.';
$strings['InvalidPaymentMethod'] = 'The selected payment method is not valid.';
$strings['ServiceNotFound'] = 'The selected service could not be found.';
$strings['ServiceInactiveForPurchase'] = 'This service is not active and cannot be purchased.';
$strings['ServiceInActiveUpsaleChainCannotBePurchased'] = 'This service cannot be purchased while %s is active because both services belong to the same upgrade path.';

// Service sale audit.
$strings['AuditHistory'] = 'Audit history';
$strings['NoAuditEntries'] = 'No audit entries have been recorded for this sale yet.';
$strings['AuditActor'] = 'Actor';
$strings['AuditSource'] = 'Source';
$strings['AuditIpAddress'] = 'IP address';
$strings['AuditData'] = 'Data';
$strings['AuditSystemSubject'] = 'System';
$strings['AuditSourceUser'] = 'User';
$strings['AuditSourceAdmin'] = 'Administrator';
$strings['AuditSourceGateway'] = 'Payment gateway';
$strings['AuditSourceSystem'] = 'System';
$strings['AuditActionServiceSaleCreated'] = 'Service order created';
$strings['AuditActionServiceSaleCompleted'] = 'Service sale completed';
$strings['AuditActionServiceSaleCancelled'] = 'Service sale cancelled';
$strings['AuditActionServiceUpgradeCompleted'] = 'Service upgrade completed';
$strings['AuditActionServiceReplacedByUpgrade'] = 'Service replaced by upgrade';
$strings['AuditActionRenewalEnabled'] = 'Automatic renewal enabled';
$strings['AuditActionRenewalCancelled'] = 'Automatic renewal cancelled';
$strings['AuditActionRenewalRestored'] = 'Automatic renewal restored';
$strings['AuditActionRenewalPaymentSucceeded'] = 'Renewal payment completed';
$strings['AuditActionRenewalPaymentFailed'] = 'Renewal payment failed';

$strings['CouponTimesApplied'] = 'Times applied';
$strings['CouponTimesAppliedHelp'] = 'Number of paid service periods that can use this coupon. Use 0 for unlimited applications.';
$strings['CouponTimesAppliedUnlimited'] = 'Unlimited';
$strings['CouponTimesAppliedMustBeZeroOrGreater'] = 'Times applied must be zero or greater.';

$strings['AllSales'] = 'All sales';
$strings['CourseSessionSales'] = 'Course and session sales';
$strings['ServiceSales'] = 'Service sales';
$strings['SubscriptionSales'] = 'Subscription sales';
$strings['SaleSource'] = 'Sales source';
$strings['ExportReportPreviewHelp'] = 'Preview of the sales that will be exported with the current filters.';
$strings['ExportReportPreviewLimited'] = 'Only the first %s rows are shown here; the Excel export includes all matching rows.';
$strings['ExportReportTotalAmount'] = 'Total amount';
$strings['NoSalesFoundForExport'] = 'No sales were found for the selected export filters.';
$strings['GatewaySubscription'] = 'Gateway subscription';
