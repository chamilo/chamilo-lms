...
#   replaced by a required multi-select `roles` field. The Vue Add User form
#   now preselects Learner (`STUDENT` option code, normalized server-side to `ROLE_STUDENT`) to preserve the legacy Student
#   default and help admins with the most common user type. Scenarios creating
#   learners therefore rely on that default; scenarios creating another role
#   explicitly remove Learner before selecting the intended role.
...
# - The manual-password field/group renders when
#   `security.admins_can_set_users_pass` is enabled. Its schema default is now
#   explicitly `true`, matching the settings UI and the intended fresh-install
#   behavior, so the HRM scenario can use a known manual password directly
#   without first toggling the setting through the settings screen.
