...
class GlossaryManager
{
    ...
    public static function get_glossary_term_by_glossary_id($glossary_id)
    {
        ...
        $glossary_table = Database::get_course_table(TABLE_GLOSSARY);
        $course_id      = api_get_course_int_id();
        $sql            = "SELECT description FROM $glossary_table WHERE c_id = $course_id  AND glossary_id =".Database::escape_string(
            $glossary_id
        );
        $rs             = Database::query($sql);
    ...
    public static function get_glossary_term_by_glossary_name($glossary_name)
    {
        ...
        $course_id      = api_get_course_int_id();
        $sql_filter     = api_get_session_condition($session_id);
        $sql            = 'SELECT description FROM '.$glossary_table.' WHERE c_id = '.$course_id.' AND name LIKE trim("'.Database::escape_string(
            $glossary_name
        ).'")'.$sql_filter;
        $rs             = Database::query($sql);
        if (Database::num_rows($rs) > 0) {
    ...
    function delete_glossary($glossary_id, $message = true)
    {
        ...
        $sql    = "DELETE FROM $t_glossary WHERE c_id = $course_id AND glossary_id='".Database::escape_string(
            $glossary_id
        )."'";
        $result = Database::query($sql);
        ...
        api_item_property_update(
            api_get_course_info(),
            TOOL_GLOSSARY,
            Database::escape_string($glossary_id),
            'delete',
            api_get_user_id()
        );
    ...
    static function display_glossary($view = 'table')
    {
        ...
        if (api_is_allowed_to_edit(null, true)) {
            echo '<a href="index.php?'.api_get_cidreq().'&action=addglossary&msg=add">'.Display::return_icon(
        ...
        echo '<a href="index.php?'.api_get_cidreq().'&action=export">'.Display::return_icon(
            'export_csv.png',
            get_lang('ExportGlossaryAsCSV'),
            '',
            '32'
        ).'</a>';
        if (api_is_allowed_to_edit(null, true)) {
            echo '<a href="index.php?'.api_get_cidreq().'&action=import">'.Display::return_icon(
                'import_csv.png',
                get_lang('ImportGlossary'),
                '',
                '32'
            ).'</a>';
        }

        echo '<a href="index.php?'.api_get_cidreq().'&action=export_to_pdf">'.Display::return_icon(
            'pdf.png',
        ...
        if ((isset($_SESSION['glossary_view']) && $_SESSION['glossary_view'] == 'table') or (!isset($_SESSION['glossary_view']))) {
            echo '<a href="index.php?'.api_get_cidreq().'&action=changeview&view=list">'.Display::return_icon(
                'view_detailed.png',
                get_lang('ListView'),
                '',
                '32'
            ...
            echo '<a href="index.php?'.api_get_cidreq().'&action=changeview&view=table">'.Display::return_icon(
                'view_text.png',
                get_lang('TableView'),
                '',
                '32'
        ...
        if (!$_SESSION['glossary_view'] OR $_SESSION['glossary_view'] == 'table') {
            $table = new SortableTable('glossary', array(
                'GlossaryManager',
                'get_number_glossary_terms'
            ), array(
                'GlossaryManager',
                'get_glossary_data'
            ), 0);
    ...
    static function actions_filter($glossary_id, $url_params, $row)
    {
        ...
        ).'?action=edit_glossary&amp;glossary_id='.$glossary_id.'&'.api_get_cidreq().'&msg=edit">'.Display::return_icon(
            'edit.png',
            get_lang('Edit'),
            '',
            22
        ...
        if (api_is_allowed_to_edit(null, true)) {
            if ($glossary_data['session_id'] == api_get_session_id()) {
                $return .= '<a href="'.api_get_self(
                ).'?action=delete_glossary&amp;glossary_id='.$glossary_id.'&'.api_get_cidreq(
                ).'" onclick="return confirmation(\''.$glossary_term.'\');">'.Display::return_icon(
                    'delete.png',
                    get_lang('Delete'),
                    '',
                    22
    ...
    function reorder_glossary()
    {
        ...
        while ($data = Database::fetch_array($res)) {
            $sql_reorder = "UPDATE $t_glossary SET display_order = $i
							WHERE c_id = $course_id  AND glossary_id = '".Database::escape_string(
                $data['glossary_id']
            )."'";
    ...
    function move_glossary($direction, $glossary_id, $message = true)
    {
        ...
        $sql1 = "UPDATE $t_glossary SET display_order = '".Database::escape_string(
            $next_display_order
        )."' WHERE c_id = $course_id  AND glossary_id = '".Database::escape_string($current_id)."'";
        $sql2 = "UPDATE $t_glossary SET display_order = '".Database::escape_string(
