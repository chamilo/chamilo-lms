    ...
    <template v-else>
      <BaseCard>
        ...
        <div class="space-y-5">
          ...
          <p
            ...
            class="text-sm text-gray-600"
          >
            {{ t("User contributions") }}: <strong>{{ reportData.userName }}</strong>
          </p>
        </div>
      </BaseCard>
      ...
      >
        <BaseCard>
          ...
          <dl class="divide-y divide-gray-20">
            ...
            >
              ...
              <dd class="text-sm font-semibold text-gray-90 sm:text-right">{{ row.value }}</dd>
            </div>
          </dl>
        </BaseCard>
        ...
        <BaseCard class="xl:col-span-2">
          <template #title>{{ t("Information about the content of the pages") }}</template>
          <div class="overflow-x-auto">
            <table class="w-full border-collapse text-sm">
              <thead>
                <tr class="border-b border-gray-30 text-left">
                  <th class="px-3 py-2 font-semibold text-gray-90">{{ t("Metric") }}</th>
                  <th class="px-3 py-2 text-right font-semibold text-gray-90">{{ t("Latest version") }}</th>
                  <th class="px-3 py-2 text-right font-semibold text-gray-90">{{ t("All versions") }}</th>
                </tr>
              </thead>
              <tbody>
                <tr
                ...
                >
                  ...
                  <td class="px-3 py-2 text-right font-medium text-gray-90">{{ row.latest }}</td>
                  <td class="px-3 py-2 text-right font-medium text-gray-90">{{ row.allVersions }}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        >
          <template #body="slotProps">
            <div class="flex flex-col gap-1">
              ...
              >
                {{ slotProps.data.categories.map((category) => category.title).join(", ") }}
              </span>
            </div>
          </template>
        </Column>
...
const isUserContributionReport = computed(() => "user-contributions" === reportData.report)
const isStatisticsReport = computed(() => "statistics" === reportData.report)
const isActiveUsersReport = computed(() => "active-users" === reportData.report)
const isWantedReport = computed(() => "wanted" === reportData.report)
const isMetricReport = computed(() =>
  ["most-changed", "most-visited", "most-linked", "orphaned"].includes(reportData.report),
)
...
const reportIcon = computed(() => {
  ...
  if ("active-users" === reportData.report || "user-contributions" === reportData.report) {
...
const reportOptions = computed(() => {
  ...
  if (isUserContributionReport.value) {
    options.unshift({ value: "user-contributions", label: t("User contributions") })
  }
...
const generalStatistics = computed(() => {
  ...
  return [
    { label: "Learners can add new pages to the Wiki", value: formatBoolean(values.learnersCanAddPages) },
    { label: "Creation date of the oldest Wiki page", value: formatDate(values.firstCreatedAt) },
    { label: "Date of most recent edition of Wiki", value: formatDate(values.lastUpdatedAt) },
    { label: "Average rating of all pages", value: formatPercent(values.averageScore) },
    { label: "Mean estimated progress by users on their pages", value: formatPercent(values.averageProgress) },
    { label: "Total users that have participated in this Wiki", value: formatNumber(values.contributors) },
    {
...
const pageStatistics = computed(() => {
  ...
  return [
    { label: "Pages", value: formatPair(values.pages, values.versions, "Versions") },
    { label: "Total of empty pages", value: formatPair(values.emptyPages, values.emptyVersions, "Versions") },
    { label: "Number of visits", value: formatPair(values.visits, values.versionVisits, "Versions") },
    { label: "Total pages edited at this time", value: formatNumber(values.editingNow) },
    { label: "Total hidden pages", value: formatNumber(values.hiddenPages) },
    { label: "Number of protected pages", value: formatNumber(values.protectedPages) },
    { label: "Number of discussion pages blocked", value: formatNumber(values.discussionLockedPages) },
    { label: "Number of discussion pages hidden", value: formatNumber(values.discussionHiddenPages) },
    { label: "Total comments on various versions of the pages", value: formatNumber(values.versionComments) },
    { label: "Total pages can only be scored by a teacher", value: formatNumber(values.teacherRatingOnlyPages) },
    { label: "Total pages that can be scored by other learners", value: formatNumber(values.learnerRatingPages) },
    { label: "Number of assignments pages proposed by a teacher", value: formatNumber(values.teacherAssignmentPages) },
    { label: "Number of individual assignments learner pages", value: formatNumber(values.learnerAssignmentPages) },
...
const contentStatistics = computed(() => {
  ...
  const rows = [
    ...
    ["Number of external html links inserted (text, images, ...).", "externalLinks"],
    ["Anchors", "anchors"],
    ...
    ["Number of audio and video files inserted (except mp3 and flv)", "multimediaFiles"],
...
function getReportParams() {
  return {
    ...
    itemsPerPage: Number(getQueryValue(route.query.itemsPerPage) || rows.value || 20),
    ...
    searchContent: "1" === String(getQueryValue(route.query.searchContent) || "0") ? 1 : undefined,
    allVersions: "1" === String(getQueryValue(route.query.allVersions) || "0") ? 1 : undefined,
    categoryIds: String(getQueryValue(route.query.categoryIds) || ""),
    matchAllCategories: "1" === String(getQueryValue(route.query.matchAllCategories) || "0") ? 1 : undefined,
...
function updateSearchCategories(value) {
  searchForm.categoryIds = Array.isArray(value) ? value.map(Number).filter((item) => item > 0) : []
}
...
function submitSearch() {
  void router.push(
    getReportRoute("search", {
      ...
      categoryIds: searchForm.categoryIds.length ? searchForm.categoryIds.join(",") : undefined,
      matchAllCategories:
        searchForm.categoryIds.length > 1 && searchForm.matchAllCategories ? 1 : undefined,
      page: 1,
    }),
  )
}
...
async function loadReport() {
  ...
  try {
    ...
    searchForm.categoryIds = Array.isArray(reportData.categoryIds) ? reportData.categoryIds.map(Number) : []
