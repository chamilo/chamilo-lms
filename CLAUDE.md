...
- **CSRF on state-changing endpoints.** Do **not** add a per-endpoint token. `CsrfProtectionListener` (`src/CoreBundle/EventListener/CsrfProtectionListener.php`) already guards every state-changing request whose route the router resolves — Symfony controllers, API Platform operations and the legacy AJAX endpoints under `public/main/inc/ajax/` alike — by verifying the request origin through `Sec-Fetch-Site`/`Origin`/`Referer`. The server emits nothing and the client sends nothing, so a new POST/PUT/PATCH/DELETE is protected as soon as its route exists; enforcement is on by default (`CHAMILO_CSRF_ENFORCE` in `config/services.yaml`). Verify these three things instead: the action is **not** a GET (the listener skips safe methods, so a sensitive GET has no protection at all — make it a POST, or give it its own token as `login_as` does); the operation does **not** declare `extraProperties: ['csrf' => false]`, which removes its only defense; and no route was added to the listener's `EXCLUDED_ROUTES`. Two categories stay outside the listener and keep their own token: legacy pages under `public/main/`, which the web server runs without a resolved route and which use `FormValidator` (whose token is only checked when the page called `protect()`), and Twig forms with `csrf_token()`. Some processors under `State/Gradebook/` and `State/AdminStatistics/` still emit and validate their own token — that is the superseded pattern, pending removal. Do not copy it.
