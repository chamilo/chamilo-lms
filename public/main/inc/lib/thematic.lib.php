...
class Thematic
{
    ...
    public function getMoveActions(int $thematicId, int $currentOrder, int $maxOrder): string
    {
        ...
        if ($currentOrder > 0) {
            $toolbarThematic .= '<a class="btn btn--secondary" href="'.api_get_self().'?action=moveup&'.api_get_cidreq().$params.'">' . Display::getMdiIcon(ActionIcon::UP, 'ch-tool-icon', null, ICON_SIZE_MEDIUM, get_lang('Up')) . '</a>';
        } else {
        ...
        if ($currentOrder < $maxOrder - 1) {
            $toolbarThematic .= '<a class="btn btn--secondary" href="'.api_get_self().'?action=movedown&'.api_get_cidreq().$params.'">' . Display::getMdiIcon(ActionIcon::DOWN, 'ch-tool-icon', null, ICON_SIZE_MEDIUM, get_lang('Down')) . '</a>';
        } else {
