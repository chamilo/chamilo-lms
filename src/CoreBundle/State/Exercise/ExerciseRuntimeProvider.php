...
final readonly class ExerciseRuntimeProvider implements ProviderInterface
{
    ...
    private function getOnlyofficeEditorUrl(Operation $operation, TrackEAttempt $attemptRow, ResourceNode $resourceNode, Course $course, ?Session $session): string
    {
        ...
        if (null === $quiz || null === $quiz->getIid() || $resourceNodeId <= 0) {
            return '';
        }
    ...
    private function buildRuntimePages(CQuiz $quiz, array $questions): array
    {
        ...
        if (!$oneQuestionPerPage) {
            ...
            $usesStructuralPages = \count($pages) > 1 || $this->containsPageBreak($questions);
